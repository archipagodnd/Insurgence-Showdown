 ...
 *********************************************************/
 ...
 const Punishments = new (_class = class {
	...
	hasPunishType(name, types, ip) {
		...
		const byName = _optionalChain([exports.Punishments, 'access', _42 => _42.userids, 'access', _43 => _43.get, 'call', _44 => _44(name), 'optionalAccess', _45 => _45.some, 'call', _46 => _46(p => types.includes(p.type))]);
		if (!ip) return byName;
		return byName || _optionalChain([exports.Punishments, 'access', _47 => _47.ipSearch, 'call', _48 => _48(ip), 'optionalAccess', _49 => _49.some, 'call', _50 => _50(p => types.includes(p.type))]);
	}

	hasRoomPunishType(room, name, types) {
		if (typeof types === 'string') types = [types];
		if (typeof (room ).roomid === 'string') room = (room ).roomid;
		return _optionalChain([exports.Punishments, 'access', _51 => _51.roomUserids, 'access', _52 => _52.nestedGet, 'call', _53 => _53(room , name), 'optionalAccess', _54 => _54.some, 'call', _55 => _55(p => types.includes(p.type))]);
	}

	__init14() {this.sortedTypes = ['TICKETBAN', 'LOCK', 'NAMELOCK', 'BAN']}
	__init15() {this.sortedRoomTypes = [...(_optionalChain([global, 'access', _56 => _56.Punishments, 'optionalAccess', _57 => _57.sortedRoomTypes]) || []), 'ROOMBAN', 'BLACKLIST']}
	...
	checkName(user, userid, registered) {
		...
		if (!punishment && user.namelocked) {
			punishment = _optionalChain([exports.Punishments, 'access', _58 => _58.userids, 'access', _59 => _59.get, 'call', _60 => _60(user.namelocked), 'optionalAccess', _61 => _61[0]]);
			if (!punishment) punishment = {type: 'NAMELOCK', id: user.namelocked, expireTime: 0, reason: ''};
		}
		if (!punishment && user.locked) {
			punishment = _optionalChain([exports.Punishments, 'access', _62 => _62.userids, 'access', _63 => _63.get, 'call', _64 => _64(user.locked), 'optionalAccess', _65 => _65[0]]);
			if (!punishment) punishment = {type: 'LOCK', id: user.locked, expireTime: 0, reason: ''};
		}

		const ticket = _optionalChain([Chat, 'access', _66 => _66.pages, 'optionalAccess', _67 => _67.help]) ?
		...
		if (id === 'NAMELOCK' || user.namelocked) {
			...
			user.locked = punishUserid;
			user.updateIdentity();
		} else if (_optionalChain([punishmentInfo, 'optionalAccess', _68 => _68.callback])) {
			...
			for (const punishment of punishments) {
				if (exports.Punishments.sharedIps.has(user.latestIp)) {
					...
					if (['BAN', 'LOCK', 'NAMELOCK'].includes(punishment.type)) {
						...
						_optionalChain([info, 'optionalAccess', _69 => _69.callback, 'optionalAccess', _70 => _70.call, 'call', _71 => _71(this, user, punishment, null, punishment.id === user.id)]);
					}
				}
			}
			if (!shared) exports.Punishments.checkPunishmentTime(user, exports.Punishments.byWeight(punishments)[0]);
		}
	...
	checkNameInRoom(user, roomid) {
		...
		if (_optionalChain([punishment, 'optionalAccess', _72 => _72.some, 'call', _73 => _73(p => p.type === 'ROOMBAN' || p.type === 'BLACKLIST')])) {
			return true;
		}
	...
	checkNewNameInRoom(user, userid, roomid) {
		...
		if (punishments) {
			for (const punishment of punishments) {
				...
				if (_optionalChain([info, 'optionalAccess', _74 => _74.callback])) {
	...
	checkLockExpiration(userid) {
		...
		const user = Users.get(userid);
		if (_optionalChain([user, 'optionalAccess', _75 => _75.permalocked])) return ` (never expires; you are permalocked)`;
	...
	getRoomPunishments(user, options = {}) {
		...
		for (const curRoom of Rooms.global.chatRooms) {
			...
			if (punishment) {
				...
				continue;
			} else if (_optionalChain([options, 'optionalAccess', _76 => _76.checkIps])) {
				if (typeof user !== 'string') {
	...
	getPunishments(roomid, ignoreMutes) {
		...
		(roomid ? exports.Punishments.roomIps.get(roomid) : exports.Punishments.ips).each((punishment, ip) => {
			...
			let entry = _optionalChain([punishmentTable, 'access', _77 => _77.find, 'call', _78 => _78(e => e[0] === id && e[1].punishType === type), 'optionalAccess', _79 => _79[1]]);
		...
		(roomid ? exports.Punishments.roomUserids.get(roomid) : exports.Punishments.userids).each((punishment, userid) => {
			...
			let entry = _optionalChain([punishmentTable, 'access', _80 => _80.find, 'call', _81 => _81(([curId, cur]) => id === curId && cur.punishType === type), 'optionalAccess', _82 => _82[1]]);
			if (!entry) {
				entry = {
					userids: [],
					ips: [],
		...
		if (roomid && ignoreMutes !== false) {
			const room = Rooms.get(roomid);
			if (_optionalChain([room, 'optionalAccess', _83 => _83.muteQueue])) {
	...
	async monitorRoomPunishments(user) {
		...
		if (punishments.length >= minPunishments) {
			...
			const punishmentText = punishments.map(([room, punishment]) => {
				...
				let punishDesc = _optionalChain([exports.Punishments, 'access', _84 => _84.roomPunishmentTypes, 'access', _85 => _85.get, 'call', _86 => _86(punishType), 'optionalAccess', _87 => _87.desc]);
				if (!punishDesc) punishDesc = `punished`;
				if (punishUserid !== userid) punishDesc += ` as ${punishUserid}`;

				// Backwards compatibility for current punishments
				const trimmedReason = _optionalChain([reason, 'optionalAccess', _88 => _88.trim, 'call', _89 => _89()]);
