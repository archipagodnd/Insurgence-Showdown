...
var _ladderschallenges = require('./ladders-challenges');
...
class Ladder extends LadderStore {
	...
	async prepBattle(connection, challengeType, team = null, isRated = false) {
		...
		return new (0, _ladderschallenges.BattleReady)(userid, this.formatid, settings, rating, challengeType);
	}
	...
	async makeChallenge(connection, targetUser) {
		...
		exports.Ladders.challenges.add(new (0, _ladderschallenges.BattleChallenge)(user.id, targetUser.id, ready));
		exports.Ladders.challenges.send(user.id, targetUser.id, `/text ${user.name} wants to battle!`);
		user.lastChallenge = Date.now();
		return true;
	}
	static async acceptChallenge(connection, chall) {
		const ladder = exports.Ladders.call(void 0, chall.format);
...
const periodicMatchInterval = setInterval(
 ...
 const Ladders = Object.assign(getLadder, {
	BattleReady: _ladderschallenges.BattleReady,
	LadderStore,
	Ladder,

	BattleChallenge: _ladderschallenges.BattleChallenge,
	GameChallenge: _ladderschallenges.GameChallenge,
	BattleInvite: _ladderschallenges.BattleInvite,
	...
	searches,
	challenges: _ladderschallenges.challenges,
 ...
 //# sourceMappingURL=sourceMaps/ladders.js.map