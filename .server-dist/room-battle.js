 ...
 class RoomBattle extends RoomGames.RoomGame {
	...
	checkPrivacySettings(options) {
		...
		if (privacySetter.size) {
			...
			} else if (!options.tour || (_optionalChain([room, 'access', _26 => _26.tour, 'optionalAccess', _27 => _27.allowModjoin]))) {
	...
	checkForcedUserSettings(user) {
		...
		if (
			this.players.some(p => _optionalChain([p, 'access', _28 => _28.getUser, 'call', _29 => _29(), 'optionalAccess', _30 => _30.battleSettings, 'access', _31 => _31.special])) ||
	...
	static battleForcedSetting(user, key) {
		...
		if (!_optionalChain([Config, 'access', _32 => _32.forcedprefixes, 'optionalAccess', _33 => _33[key]])) return null;
	...
	updatePlayer(player, user, playerOpts) {
		...
		if (user) {
			const options = {
				name: player.name,
				avatar: user.avatar,
				team: _optionalChain([playerOpts, 'optionalAccess', _34 => _34.team]),
			};
			void this.stream.write(`>player ${slot} ` + JSON.stringify(options));
	...
	start() {
		...
		const suspectTest = _optionalChain([Chat, 'access', _35 => _35.plugins, 'access', _36 => _36['suspect-tests'], 'optionalAccess', _37 => _37.suspectTests, 'access', _38 => _38[this.format]]);
		if (suspectTest) {
		...
		try {
			...
			if (battle) {
				for (const side of battle.sides) {
					if (_optionalChain([side, 'optionalAccess', _39 => _39.requestState])) {
...
if (!exports.PM.isParentProcess) {
 ...
 //# sourceMappingURL=sourceMaps/room-battle.js.map