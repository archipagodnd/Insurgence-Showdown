...
var _roomsettings = require('./chat-commands/room-settings');
...
var _roomminoractivity = require('./room-minor-activity');
 ...
 *********************************************************/
 ...
 class BasicRoom {
	...
	getMinorActivity(constructor) {
		if (_optionalChain([this, 'access', _ => _.minorActivity, 'optionalAccess', _2 => _2.constructor, 'access', _3 => _3.name]) === constructor.name) return this.minorActivity ;
		return null;
	}
	getMinorActivityQueue(settings = false) {
		const usedQueue = settings ? this.settings.minorActivityQueue : this.minorActivityQueue;
		if (!_optionalChain([usedQueue, 'optionalAccess', _4 => _4.length])) return null;
		return usedQueue;
	}
	...
	setMinorActivity(activity, noDisplay = false) {
		_optionalChain([this, 'access', _5 => _5.minorActivity, 'optionalAccess', _6 => _6.endTimer, 'call', _7 => _7()]);
	...
	saveSettings() {
		...
		if (!exports.Rooms.global) return; // during initialization
	...
	getStaffIntroMessage(user) {
		...
		if (_optionalChain([this, 'access', _8 => _8.pendingApprovals, 'optionalAccess', _9 => _9.size])) {
	...
	validateSection(section) {
		...
		if (!_roomsettings.RoomSections.sections.includes(target )) {
			throw new Chat.ErrorMessage(`"${target}" is not a valid room section. Valid categories include: ${_roomsettings.RoomSections.sections.join(', ')}`);
		}
		return target ;
	}
	setSection(section) {
		...
		if (section) {
			...
			if (oldSection === section) {
				throw new Chat.ErrorMessage(`${this.title}'s room section is already set to "${_roomsettings.RoomSections.sectionNames[oldSection]}".`);
			}
	...
	rename(newTitle, newID, noAlias) {
		...
		_optionalChain([this, 'access', _10 => _10.game, 'optionalAccess', _11 => _11.renameRoom, 'call', _12 => _12(newID)]);
	...
	onConnect(user, connection) {
		...
		_optionalChain([this, 'access', _13 => _13.minorActivity, 'optionalAccess', _14 => _14.onConnect, 'optionalCall', _15 => _15(user, connection)]);
		_optionalChain([this, 'access', _16 => _16.game, 'optionalAccess', _17 => _17.onConnect, 'optionalCall', _18 => _18(user, connection)]);
	}
	onJoin(user, connection) {
		...
		_optionalChain([this, 'access', _19 => _19.minorActivity, 'optionalAccess', _20 => _20.onConnect, 'optionalCall', _21 => _21(user, connection)]);
		_optionalChain([this, 'access', _22 => _22.game, 'optionalAccess', _23 => _23.onJoin, 'optionalCall', _24 => _24(user, connection)]);
		return true;
	}
	onRename(user, oldid, joining) {
		...
		_optionalChain([this, 'access', _25 => _25.minorActivity, 'optionalAccess', _26 => _26.onRename, 'optionalCall', _27 => _27(user, oldid, joining)]);
		this.checkAutoModchat(user);
		return true;
	}
	/**
	...
	onUpdateIdentity(user) {
		if (_optionalChain([user, 'optionalAccess', _28 => _28.connected])) {
			if (!this.users[user.id]) return false;
			if (user.named) {
	...
	runAutoModchat() {
		...
		if (!staff.length) {
			...
			if (!time || time < 5) {
				throw new Error(`Invalid time setting for automodchat (${_lib.Utils.visualize(this.settings.autoModchat)})`);
			}
	...
	checkAutoModchat(user) {
		if (user.can('mute', null, this, 'modchat')) {
			...
			if (_optionalChain([this, 'access', _29 => _29.settings, 'access', _30 => _30.autoModchat, 'optionalAccess', _31 => _31.active])) {
	...
	destroy() {
		...
		if (this.roomid === 'lobby') exports.Rooms.lobby = null;
	}
	tr(strings, ...keys) {
 ...
 class GlobalRoomState {
	...
	constructor() {
		...
		for (const [i, settings] of this.settingsList.entries()) {
			if (!_optionalChain([settings, 'optionalAccess', _32 => _32.title])) {
		...
		if (Config.logladderip) {
			this.ladderIpLog = _lib.FS.call(void 0, 'logs/ladderip/ladderip.txt').createAppendStream();
		} else {
			// Prevent there from being two possible hidden classes an instance
			// of GlobalRoom can have.
			this.ladderIpLog = new _lib.Streams.WriteStream({write() { return undefined; }});
		}
		...
		let lastBattle;
		try {
			lastBattle = _lib.FS.call(void 0, 'logs/lastbattle.txt').readSync('utf8');
		} catch (e) {}
	...
	writeChatRoomData() {
		_lib.FS.call(void 0, 'config/chatrooms.json').writeUpdate(() => (
	...
	writeNumRooms() {
		...
		_lib.FS.call(void 0, 'logs/lastbattle.txt').writeUpdate(
	...
	get configRankList() {
		...
		_lib.Utils.sortBy(rankList, rank => -typeOrder.indexOf(rank.type));
	...
	getBattles(/** formatfilter, elofilter, usernamefilter */ filter) {
		...
		for (const room of exports.Rooms.rooms.values()) {
			if (!_optionalChain([room, 'optionalAccess', _33 => _33.active]) || room.settings.isPrivate) continue;
 ...
 = {
			chat: [],
			sectionTitles: Object.values(_roomsettings.RoomSections.sectionNames),
		...
		for (const room of this.chatRooms) {
			...
			const roomData = {
				...
				section: room.settings.section ?
					(_roomsettings.RoomSections.sectionNames[room.settings.section] || room.settings.section) : undefined,
			};
	...
	startLockdown(err = null, slow = false) {
		...
		// @ts-ignore
		const stack = (err ? _lib.Utils.escapeHTML(err.stack).split(`\n`).slice(0, 2).join(`<br />`) : ``);
	...
	reportCrash(err, crasher = "The server") {
		...
		const [stackFirst, stackRest] = _lib.Utils.splitFirst(_lib.Utils.escapeHTML(stack), `<br />`);
		...
		if (devRoom) {
			...
			_optionalChain([exports.Rooms, 'access', _34 => _34.lobby, 'optionalAccess', _35 => _35.add, 'call', _36 => _36(crashMessage), 'access', _37 => _37.update, 'call', _38 => _38()]);
			_optionalChain([exports.Rooms, 'access', _39 => _39.get, 'call', _40 => _40('staff'), 'optionalAccess', _41 => _41.add, 'call', _42 => _42(crashMessage), 'access', _43 => _43.update, 'call', _44 => _44()]);
		}
 ...
 class ChatRoom extends BasicRoom {constructor(...args2) { super(...args2); ChatRoom.prototype.__init.call(this);ChatRoom.prototype.__init2.call(this);ChatRoom.prototype.__init3.call(this); }
	// This is not actually used, this is just a fake class to keep
	// TypeScript happy
	__init() {this.battle = null}
	__init2() {this.active = false}
	__init3() {this.type = 'chat'}
 ...
 class GameRoom extends BasicRoom {
	...
	constructor(roomid, title, options) {
		...
		this.p1 = _optionalChain([options, 'access', _45 => _45.p1, 'optionalAccess', _46 => _46.user]) || null;
		this.p2 = _optionalChain([options, 'access', _47 => _47.p2, 'optionalAccess', _48 => _48.user]) || null;
		this.p3 = _optionalChain([options, 'access', _49 => _49.p3, 'optionalAccess', _50 => _50.user]) || null;
		this.p4 = _optionalChain([options, 'access', _51 => _51.p4, 'optionalAccess', _52 => _52.user]) || null;
	...
	async uploadReplay(user, connection, options) {
		...
		const [success] = await LoginServer.request('prepreplay', {
			...
			inputlog: _optionalChain([battle, 'access', _53 => _53.inputLog, 'optionalAccess', _54 => _54.join, 'call', _55 => _55('\n')]) || null,
		});
		if (success) battle.replaySaved = true;
		if (_optionalChain([success, 'optionalAccess', _56 => _56.errorip])) {
...
function getRoom(roomid) {
 ...
 const Rooms = {
	MODLOG_PATH: _modlog.MODLOG_PATH,
	MODLOG_DB_PATH: _modlog.MODLOG_DB_PATH,
	Modlog: _modlog.mainModlog,
	/**
	...
	createBattle(options) {
		...
		const roomid = exports.Rooms.global.prepBattleRoom(options.format);
		...
		const battle = new exports.Rooms.RoomBattle(room, options);
		room.game = battle;
	...
	MinorActivity: _roomminoractivity.MinorActivity,
 ...
 //# sourceMappingURL=sourceMaps/rooms.js.map