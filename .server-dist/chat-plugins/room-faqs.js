...
const MAX_ROOMFAQ_LENGTH = 8192;
 ...
 const roomFaqs = (() => {
	const data = JSON.parse(_lib.FS.call(void 0, exports.ROOMFAQ_FILE).readIfExistsSync() || "{}");
	let save = false;
	for (const k in data) {
...
function saveRoomFaqs(table) {
	_lib.FS.call(void 0, exports.ROOMFAQ_FILE).writeUpdate(() => JSON.stringify(table || exports.roomFaqs));
}
...
function convertFaq(faq) {
 ...
 function getAlias(roomid, key) {
	...
	if (_optionalChain([value, 'optionalAccess', _ => _.alias])) return value.source;
	return false;
 ...
 const commands = {
	...
	removefaq(target, room, user) {
		...
		if (
			_optionalChain([room, 'access', _2 => _2.settings, 'access', _3 => _3.repeats, 'optionalAccess', _4 => _4.length]) &&
	...
	roomfaq(target, room, user, connection, cmd) {
		...
		if (!this.broadcasting && user.can('ban', null, room, 'rfaq')) {
			const code = _lib.Utils.escapeHTML(exports.roomFaqs[room.roomid][topic].source).replace(/\n/g, '<br />');
	...
	roomfaqs(args, user) {
		...
		for (const key of sortedKeys) {
			...
			if (user.can('ban', null, room, 'addfaq')) {
				const src = _lib.Utils.escapeHTML(topic.source).replace(/\n/g, `<br />`);
