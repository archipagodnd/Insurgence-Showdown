...
const REPLAY_REGEX = new RegExp(
	`${_lib.Utils.escapeRegex(Config.routes.replays)}/(?:[a-z0-9]-)?(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]+pw)?`, "g"
);
...
function writeStats(line) {
 ...
 class HelpTicket extends Rooms.RoomGame {
	...
	constructor(room, ticket) {
		super(room);
		this.room = room;
		this.room.settings.language = _optionalChain([Users, 'access', _ => _.get, 'call', _2 => _2(ticket.creator), 'optionalAccess', _3 => _3.language]) || 'english' ;
	...
	getPreview() {
		...
		const noteBuf = Object.entries(this.ticket.notes || {})
			.map(([userid, note]) => _lib.Utils.html`${note} (by ${userid})`)
			.join('&#10;');
	...
	close(result, staff) {
		...
		exports.tickets[this.ticket.userid] = this.ticket;
		writeTickets();
		this.room.modlog({action: 'TICKETCLOSE', isGlobal: false, loggedBy: _optionalChain([staff, 'optionalAccess', _4 => _4.id]) || 'unknown' });
	...
	static __initStatic() {this.modlogStream = Rooms.Modlog.initialize('help-texttickets' )}
	// workaround to modlog for no room
	static modlog(entry) {
		Rooms.Modlog.write('help-texttickets' , entry);
	}
	static logTextResult(ticket) {
		...
		void _lib.FS.call(void 0, `logs/tickets/${date.slice(0, -3)}.jsonl`).append(JSON.stringify(entry) + '\n');
	}
	static async getTextLogs(userid, date) {
		const results = [];
		if (await _configloader.checkRipgrepAvailability.call(void 0, )) {
			const args = [`-e`, `userid":"${userid}`, '--no-filename'];
			let lines;
			try {
				lines = await _lib.ProcessManager.exec([
			...
			const path = _lib.FS.call(void 0, `logs/tickets/${date}.jsonl`);
			if (!path.existsSync()) {
	...
	static formatBattleLog(logs, title, url) {
		const log = logs.filter(l => l.startsWith('|c|'));
		let buf = ``;
		for (const line of log) {
			const [,, username, message] = _lib.Utils.splitFirst(line, '|', 3);
			buf += _lib.Utils.html`<div class="chat"><span class="username"><username>${username}:</username></span> ${message}</div>`;
		}
		if (buf) buf = `<div class="infobox"><strong><a href="${url}">${title}</a></strong><hr />${buf}</div>`;
		return buf;
	}
	static async visualizeBattleLogs(rooms) {
		const logs = [];
	...
	static getTextButton(ticket) {
		...
		const title = `title="${titleBuf.map(_lib.Utils.escapeHTML).join('&#10;')}${notes}"`;
		const language = _optionalChain([Users, 'access', _6 => _6.get, 'call', _7 => _7(ticket.userid), 'optionalAccess', _8 => _8.language]) || '';
	...
	static notifyResolved(user, ticket, userid = user.id) {
		const {result, time, by, seen} = ticket.resolved ;
		if (seen) return;
		...
		if (_optionalChain([result, 'optionalAccess', _9 => _9.trim, 'call', _10 => _10()])) {
	...
	static getTypeId(name) {
		return _optionalChain([Object, 'access', _11 => _11.entries, 'call', _12 => _12(ticketTitles), 'access', _13 => _13.find, 'call', _14 => _14(entry => entry[1] === name), 'optionalAccess', _15 => _15[0]]) || toID(name);
	}
} HelpTicket.__initStatic(); exports.HelpTicket = HelpTicket;
...
function notifyUnclaimedTicket(hasAssistRequest) {
 ...
 function notifyStaff() {
	...
	if (!room) return;
	let buf = ``;
	const sortedTickets = _lib.Utils.sortBy(Object.values(exports.tickets), ticket => [
		!ticket.offline,
		ticket.open,
	...
	for (const ticket of sortedTickets) {
		...
		const ticketRoom = Rooms.get(`help-${ticket.userid}`);
		const ticketGame = _optionalChain([ticketRoom, 'optionalAccess', _16 => _16.getGame, 'call', _17 => _17(HelpTicket)]);
		if (!ticket.claimed) {
			hasUnclaimed = true;
	...
	for (const user of Object.values(room.users)) {
		...
		for (const connection of user.connections) {
			if (_optionalChain([connection, 'access', _18 => _18.openPages, 'optionalAccess', _19 => _19.has, 'call', _20 => _20('help-tickets')])) {
...
function checkIp(ip) {
	for (const t in exports.tickets) {
		if (exports.tickets[t].ip === ip && exports.tickets[t].open && !Punishments.sharedIps.has(ip)) {
 ...
 async function getOpponent(link, submitter) {
	...
	if (_optionalChain([room, 'optionalAccess', _21 => _21.battle])) {
	...
	if (!room) {
		const replayUrl = _lib.Net.call(void 0, `https://${Config.routes.replays}/${link.slice(link.indexOf('-') + 1)}.json`);
		try {
 ...
 async function getBattleLog(battle) {
	...
	try {
		const raw = await _lib.Net.call(void 0, `https://${Config.routes.replays}/${battle}.json`).get();
		const data = JSON.parse(raw);
		if (_optionalChain([data, 'access', _22 => _22.log, 'optionalAccess', _23 => _23.length])) {
			return {
...
function refreshPageFor(page, roomid, ignoreUsers) {
	const room = Rooms.get(roomid);
	if (room) {
		for (const curUser of Object.values(room.users)) {
			if (_optionalChain([ignoreUsers, 'optionalAccess', _24 => _24.includes, 'call', _25 => _25(curUser.id)])) continue;
			for (const conn of curUser.connections) {
				if (_optionalChain([conn, 'access', _26 => _26.openPages, 'optionalAccess', _27 => _27.has, 'call', _28 => _28(page)])) {
...
const cheatingScenarios = [
 ...
 const textTickets = {
	pmharassment: {
		...
		async getReviewDisplay(ticket, staff, conn) {
			...
			const sharedBattles = _info.getCommonBattles.call(void 0, ticket.userid, null, reportUserid, null, conn);
		...
		onSubmit(ticket, text, submitter, conn) {
			...
			_info.getCommonBattles.call(void 0, targetId, Users.get(targetId), submitter.id, submitter, conn);
		},
	},
	inapname: {
		...
		getReviewDisplay(ticket) {
			...
			if (_optionalChain([ticket, 'access', _29 => _29.meta, 'optionalAccess', _30 => _30.startsWith, 'call', _31 => _31('user-')])) {
		...
		onSubmit(ticket, text) {
			if (!_optionalChain([ticket, 'access', _32 => _32.meta, 'optionalAccess', _33 => _33.startsWith, 'call', _34 => _34('user-')])) {
	...
	inappokemon: {
		...
		async getReviewDisplay(ticket, staff, conn) {
			...
			const battleRooms = links.map(r => Rooms.get(r)).filter(room => _optionalChain([room, 'optionalAccess', _35 => _35.battle])) ;
	...
	ipappeal: {
		...
		async getReviewDisplay(ticket, staff, conn, state) {
			...
			const ips = state ? state.ips : tarUser ? tarUser.ips : ticket.meta.split('-');
			...
			for (const [i, ip] of ips.entries()) {
				...
				if (ipPunishments) {
					const str = ipPunishments.map(p => (
						`${_optionalChain([Punishments, 'access', _36 => _36.punishmentTypes, 'access', _37 => _37.get, 'call', _38 => _38(p.type), 'optionalAccess', _39 => _39.desc]) || p.type} as ${p.id}${p.reason ? ` (${p.reason})` : ''}`
					));
				...
				if (ipPunishments) {
					...
					buf += `<button class="button" name="send" value="/msgroom staff,${unlockCmd}&#10;/join view-help-text-${ticket.userid}">Unlock IP</button>`;
				}
				buf += `</details>`;
			}
...
}; exports.textTickets = textTickets;
 ...
 const pages = {
	help: {
		request(query, user, connection) {
			...
			if (_optionalChain([ticket, 'optionalAccess', _40 => _40.open]) || ipTicket) {
				...
				case 'reason':
					...
					for (const [idx, punishment] of punishments.entries()) {
						if (punishments.indexOf(punishment) !== idx) {
							...
							buf += _lib.Utils.html`<p>Your ${punishment.type} was for: ${punishment.reason}.</p>`;
						}
					}
					break;
				case 'startedit':
		...
		tickets(query, user, connection) {
			...
			for (const ticket of sortedTickets) {
				...
				const ticketRoom = Rooms.get(`help-${ticket.userid}`);
				buf += `<tr><td>${icon}</td>`;
				const language = Chat.languages.get(_optionalChain([ticketRoom, 'optionalAccess', _41 => _41.settings, 'access', _42 => _42.language]) || '');
				const languageString = language && toID(language) !== "english" ? ` <small>(${language})</small>` : '';
				buf += `<td>${_lib.Utils.escapeHTML(ticket.creator)}${languageString}</td>`;
			...
			const ticketBans = _lib.Utils.sortBy(
		...
		async text(query, user, connection) {
			...
			if (ticket.notes) {
				...
				for (const staff in ticket.notes) {
					buf += _lib.Utils.html`<p>${ticket.notes[staff]} (by ${staff})</p>`;
				}
				buf += `</details></div>`;
			}
			...
			if (!ticket.resolved) {
				...
				buf += _lib.Utils.html`<strong>Resolved: by ${ticket.resolved.by}</strong><br />`;
				buf += _lib.Utils.html`<strong>Result:</strong> ${Chat.collapseLineBreaksHTML(ticket.resolved.result)}<br />`;
				if (ticket.resolved.staffReason.includes('PROOF')) { // a note was added, show it
					buf += _lib.Utils.html`<strong>Resolver notes:</strong> ${Chat.collapseLineBreaksHTML(ticket.resolved.staffReason)}<br />`;
				}
			}
			return buf;
		},
		async logs(query, user, connection) {
			...
			_lib.Utils.sortBy(logs, log => -log.date);
		...
		stats(query, user, connection) {
			...
			if (table === 'tickets') {
				...
				const sortedStats = _lib.Utils.sortBy(Object.keys(typeStats), t => {
					if (col === 'type') {
						// Alphabetize strings
						return t;
				...
				const sortedStaff = _lib.Utils.sortBy(Object.keys(staffStats), staff => {
					if (col === 'staff') {
						// Alphabetize strings
						return staff;
					}
 ...
 const commands = {
	report(target, room, user) {
		...
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _43 => _43.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	appeal(target, room, user) {
		...
		const meta = this.pmTarget ? `-user-${this.pmTarget.id}` : this.room ? `-room-${this.room.roomid}` : '';
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _44 => _44.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	helpticket: {
		...
		async submit(target, room, user, connection, cmd) {
			...
			let ticket = exports.tickets[user.id];
			const ipTicket = checkIp(user.latestIp);
			if (_optionalChain([ticket, 'optionalAccess', _45 => _45.open]) || ipTicket) {
				if (!ticket && ipTicket) ticket = ipTicket;
				if (ticket.text) {
			...
			] = _lib.Utils.splitFirst(target, '|', 4).map(s => s.trim());
			...
			if (textTicket) {
				...
				const validation = await _optionalChain([textTicket, 'access', _46 => _46.checker, 'optionalCall', _47 => _47(text, contextString || '', ticket.type, user, reportTarget)]);
				...
				writeTickets();
				notifyStaff();
				_optionalChain([textTicket, 'access', _48 => _48.onSubmit, 'optionalCall', _49 => _49(ticket, [text, contextString], this.user, this.connection)]);
			...
			if (reportTargetType === 'room') {
				...
				if (reportRoom && (reportRoom ).uploadReplay) {
				...
				if (targetID !== ticket.userid) {
					const commonBattles = _info.getCommonBattles.call(void 0, 
		...
		resolve(target, room, user) {
			this.checkCan('lock');
			const [ticketerName, result] = _lib.Utils.splitFirst(target, ',').map(i => i.trim());
		...
		addnote(target, room, user) {
			...
			const [ticketName, note] = _lib.Utils.splitFirst(target, ',').map(i => i.trim());
		...
		removenote(target, room, user) {
			...
			let [ticketName, staff] = _lib.Utils.splitFirst(target, ',').map(i => i.trim());
		...
		close(target, room, user) {
			...
			if (!_optionalChain([ticket, 'optionalAccess', _50 => _50.open]) || (ticket.userid !== user.id && !user.can('lock'))) {
		...
		async ban(target, room, user) {
			...
			for (const userObj of affected) {
				...
				if (_optionalChain([targetTicket, 'optionalAccess', _51 => _51.open])) targetTicket.open = false;
				const helpRoom = Rooms.get(`help-${userObjID}`);
				if (helpRoom) {
					const ticketGame = helpRoom.getGame(HelpTicket);
					ticketGame.writeStats('ticketban');
					helpRoom.destroy();
				} else if (_optionalChain([targetTicket, 'optionalAccess', _52 => _52.text])) {
					HelpTicket.modlog({
						action: `TICKETBAN`,
						loggedBy: user.id,
					...
					if (staffRoom) {
						for (const curUser of Object.values(staffRoom.users)) {
							for (const conn of curUser.connections) {
								if (_optionalChain([conn, 'access', _53 => _53.openPages, 'optionalAccess', _54 => _54.has, 'call', _55 => _55(`help-text-${userObjID}`)])) {
		...
		unban(target, room, user) {
			...
			this.checkCan('lock');
			target = toID(target);
			const targetID = _optionalChain([Users, 'access', _56 => _56.get, 'call', _57 => _57(target), 'optionalAccess', _58 => _58.id]) || target ;
			...
			_optionalChain([Users, 'access', _59 => _59.get, 'call', _60 => _60(target), 'optionalAccess', _61 => _61.popup, 'call', _62 => _62(`${user.name} has ticket unbanned you.`)]);
		},
		...
		logs(target, room, user) {
			this.checkCan('lock');
			const [targetString, dateString] = _lib.Utils.splitFirst(target, ',').map(i => i.trim());
 ...
 const punishmentfilter = (user, punishment) => {
	...
	if (typeof user === 'object') {
		...
		if (_optionalChain([helpRoom, 'optionalAccess', _63 => _63.game, 'optionalAccess', _64 => _64.gameid]) !== 'helpticket') return;
 ...
 const loginfilter = (user) => {
	...
	if (_optionalChain([ticket, 'optionalAccess', _65 => _65.resolved])) {
 ...
 const handlers = {
	onRoomClose(room, user, conn, isPage) {
		...
		if (_optionalChain([ticket, 'optionalAccess', _66 => _66.open]) && ticket.claimed === user.id) {
			ticket.claimed = null;
			writeTickets();
			notifyStaff();
		}
