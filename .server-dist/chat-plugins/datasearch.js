...
const dexesHelp = Object.keys((_optionalChain([global, 'access', _ => _.Dex, 'optionalAccess', _2 => _2.dexes]) || {})).filter(x => x !== 'sourceMaps').join('</code>, <code>');
...
function checkCanAll(room) {
 ...
 const commands = {
	...
	async learn(target, room, user, connection, cmd, message) {
		...
		const GENS = {rby: 1, gsc: 2, adv: 3, dpp: 4, bw2: 5, oras: 6, usum: 7};
...
function runDexsearch(target, cmd, canAll, message, isTest) {
	const searches = [];
	...
	const accumulateKeyCount = (count, searchData) =>
		count + (typeof searchData === 'object' ? Object.keys(searchData).length : 0);
	_lib.Utils.sortBy(searches, search => (
	...
	for (const alts of searches) {
		...
		for (const mon in dex) {
			...
			if (alts.tiers && Object.keys(alts.tiers).length) {
				...
				if (tier.startsWith('(') && tier !== '(PU)' && tier !== '(NU)') tier = tier.slice(1, -1) ;
				// if (tier === 'New') tier = 'OU';
			...
			for (const targetResist in alts.resists) {
				...
				if (notImmune && !move.ohko && move.damage === undefined) {
					for (const defenderType of dex[mon].types) {
						...
						const moveMod = _optionalChain([move, 'access', _3 => _3.onEffectiveness, 'optionalAccess', _4 => _4.call, 'call', _5 => _5(
							{dex: mod} , baseMod, null, defenderType, move ,
						)]);
			...
			for (const targetWeak in alts.weak) {
				...
				if (notImmune && !move.ohko && move.damage === undefined) {
					for (const defenderType of dex[mon].types) {
						const baseMod = mod.getEffectiveness(attackingType, defenderType);
						const moveMod = _optionalChain([move, 'access', _6 => _6.onEffectiveness, 'optionalAccess', _7 => _7.call, 'call', _8 => _8(
							{dex: mod} , baseMod, null, defenderType, move ,
						)]);
						effectiveness += typeof moveMod === 'number' ? moveMod : baseMod;
					}
			...
			const validator = _teamvalidator.TeamValidator.get(
	...
	if (randomOutput && randomOutput < results.length) {
		results = _lib.Utils.shuffle(results).slice(0, randomOutput);
	}
	...
	if (results.length > 1) {
		...
		if (sort) {
			...
			_lib.Utils.sortBy(results, name => {
...
function runMovesearch(target, cmd, canAll, message, isTest) {
	const searches = [];
	...
	const getFullLearnsetOfPokemon = (species) => {
		let usedSpecies = _lib.Utils.deepClone(species);
		let usedSpeciesLearnset = _lib.Utils.deepClone(mod.species.getLearnset(usedSpecies.id));
		if (!usedSpeciesLearnset) {
			usedSpecies = _lib.Utils.deepClone(mod.species.get(usedSpecies.baseSpecies));
			usedSpeciesLearnset = _lib.Utils.deepClone(mod.species.getLearnset(usedSpecies.id) || {});
		}
		const lsetData = new Set(Object.keys(usedSpeciesLearnset));

		while (usedSpecies.prevo) {
			usedSpecies = _lib.Utils.deepClone(mod.species.get(usedSpecies.prevo));
			usedSpeciesLearnset = _lib.Utils.deepClone(mod.species.getLearnset(usedSpecies.id));
		...
		for (const moveid in dex) {
			...
			for (const recoveryType in alts.other) {
				...
				if (recoveryType === "recovery") {
					...
					hasRecovery = (_optionalChain([move, 'access', _9 => _9.zMove, 'optionalAccess', _10 => _10.effect]) === 'heal');
				}
			...
			for (const lower in alts.lower) {
				if (move.boosts && move.boosts !== false) {
					...
					if (move.secondary.boosts) {
						...
						if ((move.secondary.self.boosts[lower ] < 0) === alts.lower[lower]) {
							matched = true;
							break;
						}
					}
			...
			for (const boost in alts.zboost) {
				...
				if (_optionalChain([zMove, 'optionalAccess', _11 => _11.boost])) {
	...
	if (results.length > 1) {
		...
		if (sort) {
			...
			const direction = sort.slice(-1);
			_lib.Utils.sortBy(results, moveName => {
...
function runAbilitysearch(target, cmd, canAll, message) {
...
function runLearn(target, cmd, canAll, formatid) {
	...
	const validator = _teamvalidator.TeamValidator.get(format);
...
if (!exports.PM.isParentProcess) {
	...
	// @ts-ignore
	require('../../.lib-dist/repl').Repl.start('dexsearch', cmd => eval(cmd)); // eslint-disable-line no-eval
} else {
	exports.PM.spawn(MAX_PROCESSES);
}

 const testables = {
	runDexsearch: (target, cmd, canAll, message) =>
		runDexsearch(target, cmd, canAll, message, true),
	runMovesearch: (target, cmd, canAll, message) =>
		runMovesearch(target, cmd, canAll, message, true),
}; exports.testables = testables;

 //# sourceMappingURL=sourceMaps/datasearch.js.map