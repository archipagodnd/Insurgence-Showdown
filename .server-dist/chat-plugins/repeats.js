...
var _roomfaqs = require('./room-faqs');
	...
	runRepeat(room, repeat) {
		...
		const repeater = (targetRoom) => {
			...
			const repeatedPhrase = repeat.faq ?
				_roomfaqs.visualizeFaq.call(void 0, _roomfaqs.roomFaqs[targetRoom.roomid][repeat.id]) : Chat.formatText(phrase, true);
 ...
 const pages = {
	repeats(args, user) {
		...
		for (const repeat of room.settings.repeats) {
			...
			const repeatText = repeat.faq ? _roomfaqs.roomFaqs[room.roomid][repeat.id].source : repeat.phrase;
 ...
 const commands = {
	...
	repeatfaq(target, room, user, connection, cmd) {
		...
		const faq = _roomfaqs.roomFaqs[room.roomid][topic];
		if (!faq) {
