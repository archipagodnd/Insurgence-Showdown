 ...
 const Friends = new class {
	async notifyPending(user) {
		...
		if (pendingCount === 1) {
			...
			_friends.sendPM.call(void 0, _utils.Utils.html`/raw <span class="username">${senderName}</span> sent you a friend request!`, user.id);
			_friends.sendPM.call(void 0, buf, user.id);
			_friends.sendPM.call(void 0, 
			...
			_friends.sendPM.call(void 0, `/nonotify You have ${pendingCount} friend requests pending!`, user.id);
			_friends.sendPM.call(void 0, `/raw <button class="button" name="send" value="/j view-friends-received">View</button></div>`, user.id);
		}
	}
	...
	async visualizeList(userid) {
		...
		for (const {friend: friendID, last_login, allowing_login: hideLogin} of [...friends].sort()) {
			...
			if (_optionalChain([friend, 'optionalAccess', _6 => _6.connected])) {
	...
	displayFriend(userid, login) {
		...
		const name = _utils.Utils.escapeHTML(user ? user.name : userid);
		const statusType = _optionalChain([user, 'optionalAccess', _7 => _7.connected]) ?
		...
		let buf = user ?
			...
			_utils.Utils.html`<i>${name}</i> <small>(${statusType})</small>`;
		buf += `<br />`;

		const curUser = Users.get(userid); // might be an alt
		if (user) {
			if (user.userMessage) buf += _utils.Utils.html`Status: <i>${user.userMessage}</i><br />`;
		} else if (curUser && curUser.id !== userid) {
			buf += `<small>On an alternate account</small><br />`;
		}
		if (login && typeof login === 'number' && !_optionalChain([user, 'optionalAccess', _8 => _8.connected])) {
...
function headerButtons(type, user) {
 ...
 const commands = {
	...
	friends: {
		...
		async accept(target, room, user, connection) {
			...
			const targetUser = Users.get(target);
			_friends.sendPM.call(void 0, `You accepted a friend request from "${target}".`, user.id);
		...
		async reject(target, room, user, connection) {
			...
			target = toID(target);
			if (!target) return this.parse('/help friends');
			await exports.Friends.removeRequest(user.id, target );
			this.refreshPage('friends-received');
			return _friends.sendPM.call(void 0, `You denied a friend request from '${target}'.`, user.id);
		},
		...
		async undorequest(target, room, user, connection) {
			...
			target = toID(target);
			if (user.settings.blockFriendRequests) {
				return _friends.sendPM.call(void 0, 
			...
			this.refreshPage('friends-sent');
			return _friends.sendPM.call(void 0, `You removed your friend request to '${target}'.`, user.id);
		},
 ...
 const pages = {
	async friends(args, user) {
		...
		case 'outgoing': case 'sent':
			...
			buf += `<h3>You have ${Chat.count(sent.size, 'friend requests')} pending${sent.size === _friends.MAX_REQUESTS ? ` (maximum reached)` : ''}.</h3>`;
 ...
 const loginfilter = async user => {
	...
	await exports.Friends.notifyPending(user);

	// (quietly) notify their friends (that have opted in) that they are online
	await exports.Friends.notifyConnection(user);
	// write login time
