...
function readFile(path) {
	try {
		const json = _lib.FS.call(void 0, path).readIfExistsSync();
		if (!json) {
			return false;
		}
...
for (const section of tables) {
	...
	if (Object.keys(logs[section]).length >= 3) {
		...
		const keys = _lib.Utils.sortBy(Object.keys(logs[section]), key => {
...
class MafiaPlayer extends Rooms.RoomGamePlayer {
	...
	constructor(user, game) {
		super(user, game);
		this.game = game;
		this.safeName = _lib.Utils.escapeHTML(this.name);
		this.role = null;
		this.lynching = '';
	...
	updateHtmlRoom() {
		...
		if (!_optionalChain([user, 'optionalAccess', _ => _.connected])) return;
	...
	updateHtmlLynches() {
		const user = Users.get(this.id);
		if (!_optionalChain([user, 'optionalAccess', _2 => _2.connected])) return;
...
class Mafia extends Rooms.RoomGame {
	...
	constructor(room, host) {
		...
		this.hostid = host.id;
		this.host = _lib.Utils.escapeHTML(host.name);
		this.cohostids = [];
		this.cohosts = [];
	...
	setRoles(user, roleString, force = false, reset = false) {
		...
		if (force) {
			this.originalRoles = roles.map(r => ({
				name: r,
				safeName: _lib.Utils.escapeHTML(r),
				id: toID(r),
				alignment: 'solo',
				image: '',
				memo: [`To learn more about your role, PM the host (${this.host}).`],
			}));
			_lib.Utils.sortBy(this.originalRoles, role => role.name);
		...
		_lib.Utils.sortBy(this.originalRoles, role => [role.alignment, role.name]);
		this.roles = this.originalRoles.slice();
		this.originalRoleString = this.originalRoles.map(
			r => `<span style="font-weight:bold;color:${MafiaData.alignments[r.alignment].color || '#FFF'}">${r.safeName}</span>`
		).join(', ');
	...
	static parseRole(roleString) {
		...
		const role = {
			name: roleName,
			safeName: _lib.Utils.escapeHTML(roleName),
			id: toID(roleName),
			image: '',
	...
	distributeRoles() {
		const roles = _lib.Utils.shuffle(this.roles.slice());
		if (roles.length) {
			for (const p in this.playerTable) {
				...
				if (_optionalChain([u, 'optionalAccess', _3 => _3.connected])) {
					u.send(`>${this.room.roomid}\n|notify|Your role is ${role.safeName}. For more details of your role, check your Role PM.`);
				}
			}
		}
		this.dead = {};
		this.played = [this.hostid, ...this.cohostids, ...(Object.keys(this.playerTable) )];
		this.sendDeclare(`The roles have been distributed.`);
		this.updatePlayers();
	}

	getPartners(alignment, player) {
		if (!_optionalChain([player, 'optionalAccess', _4 => _4.role]) || ['town', 'solo', 'traitor'].includes(player.role.alignment)) return "";
		const partners = [];
		for (const p in this.playerTable) {
	...
	night(early = false, initial = false) {
		...
		for (const hostid of [...this.cohostids, this.hostid]) {
			...
			if (_optionalChain([host, 'optionalAccess', _5 => _5.connected])) host.send(`>${this.room.roomid}\n|notify|It's night in your game of Mafia!`);
		}
		if (this.takeIdles) {
	...
	lynch(userid, target) {
		...
		if (this.getHammerValue(target) <= lynch.trueCount) {
			// HAMMER
			this.sendDeclare(`Hammer! ${target === 'nolynch' ? 'Nobody' : _lib.Utils.escapeHTML(name)} was voted out!`);
	...
	unlynch(userid, force = false) {
		...
		if (!player && this.dead[userid] && this.dead[userid].restless) player = this.dead[userid];
		if (!_optionalChain([player, 'optionalAccess', _6 => _6.lynching])) return this.sendUser(userid, `|error|You are not voting anyone.`);
	...
	lynchBox() {
		...
		const list = _lib.Utils.sortBy(Object.entries(this.lynches), ([key, lynch]) => [
			key === plur,
			-lynch.count,
		]);
		for (const [key, lynch] of list) {
			buf += `${lynch.count}${plur === key ? '*' : ''} ${_optionalChain([this, 'access', _7 => _7.playerTable, 'access', _8 => _8[key], 'optionalAccess', _9 => _9.safeName]) || 'No Vote'} (${lynch.lynchers.map(a => _optionalChain([this, 'access', _10 => _10.playerTable, 'access', _11 => _11[a], 'optionalAccess', _12 => _12.safeName]) || a).join(', ')})<br />`;
		}
		return buf;
	}
	lynchBoxFor(userid) {
	...
	getPlurality() {
		...
		for (const [key, lynch] of Object.entries(this.lynches)) {
			if (lynch.count > max) {
				max = lynch.count;
				topLynches = [[key , lynch]];
			} else if (lynch.count === max) {
				topLynches.push([key , lynch]);
			}
		}
		...
		topLynches = _lib.Utils.sortBy(topLynches, ([key, lynch]) => [
			lynch.dir === 'down',
	...
	revive(user, toRevive, force = false) {
		...
		if (toRevive in this.dead) {
			...
			_lib.Utils.sortBy(this.roles, r => [r.alignment, r.name]);
			...
			this.sendDeclare(_lib.Utils.html`${targetUser.name} has been added to the game by ${user.name}!`);
		}
		this.playerCount++;
		this.updateRoleString();
		this.updatePlayers();
	...
	sub(player, replacement) {
		...
		if (_optionalChain([newUser, 'optionalAccess', _13 => _13.connected])) {
	...
	nextSub(userid = null) {
		...
		if (!_optionalChain([sub, 'optionalAccess', _14 => _14.connected]) || !sub.named || !this.room.users[sub.id]) return; // should never happen, just to be safe
	...
	customIdeaInit(user, choices, picks, rolesString) {
		...
		const roles = _lib.Utils.stripHTML(rolesString);
	...
	ideaDistributeRoles(user) {
		...
		for (const p in this.playerTable) {
			...
			const u = Users.get(p);
			if (_optionalChain([u, 'optionalAccess', _15 => _15.connected])) u.send(`>${this.room.roomid}\n|notify|Pick your role in the IDEA module.`);
		}
	...
	ideaPick(user, selection) {
		...
		if (!_optionalChain([this, 'access', _16 => _16.IDEA, 'optionalAccess', _17 => _17.data])) {
	...
	ideaFinalizePicks() {
		if (!_optionalChain([this, 'access', _18 => _18.IDEA, 'optionalAccess', _19 => _19.data])) {
	...
	updateHost(...hosts) {
		...
		for (const hostid of hosts) {
			const host = Users.get(hostid);
			if (!_optionalChain([host, 'optionalAccess', _20 => _20.connected])) return;
	...
	canJoin(user, self = false, force = false) {
		if (!_optionalChain([user, 'optionalAccess', _21 => _21.connected])) return `User not found.`;
	...
	sendUser(user, message) {
		...
		if (!_optionalChain([userObject, 'optionalAccess', _22 => _22.connected])) return;
 ...
 const pages = {
	mafia(query, user) {
		...
		const game = _optionalChain([room, 'optionalAccess', _23 => _23.getGame, 'call', _24 => _24(Mafia)]);
		if (!_optionalChain([room, 'optionalAccess', _25 => _25.users, 'access', _26 => _26[user.id]]) || !game || game.ended) {
			return this.close();
		}
		...
		if (isHost) {
			...
			for (const p in game.playerTable) {
				...
				buf += game.lynchModifiers[p] !== undefined ? `(votes worth ${game.getLynchValue(p )})` : '';
			...
			for (const d in game.dead) {
				...
				if (dead.treestump) buf += ` (is a Treestump)`;
				if (dead.restless) buf += ` (is a Restless Spirit)`;
				if (game.lynchModifiers[d] !== undefined) buf += ` (votes worth ${game.getLynchValue(d )})`;
	...
	mafialadder(query, user) {
		...
		const entries = _lib.Utils.sortBy(Object.entries(logs[section][month]), ([key, value]) => (
			-value
		));
	...
	mafia: {
		...
		host(target, room, user, connection, cmd) {
			...
			if (nextHost) {
				...
				while ((hostid = hostQueue.shift())) {
					...
					if (!_optionalChain([targetUser, 'optionalAccess', _27 => _27.connected]) ||
			...
			if (!_optionalChain([targetUser, 'optionalAccess', _28 => _28.connected])) {
		...
		queue(target, room, user) {
			...
			case 'add':
				...
				if ((!_optionalChain([targetUser, 'optionalAccess', _29 => _29.connected])) && !command.includes('force')) {
		...
		customidea(target, room, user) {
			...
			const [options, roles] = _lib.Utils.splitFirst(target, '\n');
		...
		rolelist(target, room, user, connection, cmd) {
			...
			const roleString = _lib.Utils.sortBy((showOrl ? game.originalRoles : game.roles), role => (
				role.alignment
		...
		subhost(target, room, user, connection, cmd) {
			...
			if (cmd.includes('cohost')) {
				...
				game.cohosts.push(_lib.Utils.escapeHTML(targetUser.name));
				game.sendDeclare(_lib.Utils.html`${targetUser.name} has been added as a cohost by ${user.name}`);
				for (const conn of targetUser.connections) {
					void Chat.resolvePage(`view-mafia-${room.roomid}`, targetUser, conn);
				}
				...
				const queueIndex = hostQueue.indexOf(targetUser.id);
				if (queueIndex > -1) hostQueue.splice(queueIndex, 1);
				game.host = _lib.Utils.escapeHTML(targetUser.name);
				game.hostid = targetUser.id;
				game.played.push(targetUser.id);
				for (const conn of targetUser.connections) {
					void Chat.resolvePage(`view-mafia-${room.roomid}`, targetUser, conn);
				}
				game.sendDeclare(_lib.Utils.html`${targetUser.name} has been substituted as the new host, replacing ${oldHostid}.`);
		...
		removecohost(target, room, user) {
			...
			game.sendDeclare(_lib.Utils.html`${target} was removed as a cohost by ${user.name}`);
		...
		data(target, room, user, connection, cmd) {
			if (_optionalChain([room, 'optionalAccess', _30 => _30.settings, 'access', _31 => _31.mafiaDisabled])) return this.errorReply(`Mafia is disabled for this room.`);
			...
			// hack to let hosts broadcast
			const game = _optionalChain([room, 'optionalAccess', _32 => _32.getGame, 'call', _33 => _33(Mafia)]);
		...
		addtheme(target, room, user, connection, cmd) {
			...
			for (const rolelist of rolelists) {
				const [players, roles] = _lib.Utils.splitFirst(rolelist, ':', 2).map(e => e.trim());
		...
		listdata(target, room, user) {
			...
			const dataSource = MafiaData[target ];
			if (dataSource === MafiaData.aliases) {
...
process.nextTick(() => {
 ...
 //# sourceMappingURL=sourceMaps/mafia.js.map