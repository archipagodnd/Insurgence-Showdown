...
function battleFactorySets(species, tier, gen = 'gen7', isBSS = false) {
	species = Dex.species.get(species);
...
function generateSSBSet(set, dex, baseDex) {
	if (set.skip) {
		const baseSet = toID(Object.values(_randomteams.ssbSets[set.skip]).join());
...
function SSBSets(target) {
	...
	if (!Object.keys(_randomteams.ssbSets).map(toID).includes(toID(target))) {
		return {e: `Error: ${target.trim()} doesn't have a [Gen 8] Super Staff Bros 4 set.`};
	}
	let displayName = '';
	const names = [];
	for (const member in _randomteams.ssbSets) {
	...
	for (const name of names) {
		...
		if (!set.skip || set.signatureMove !== _randomteams.ssbSets[set.skip].signatureMove) {
 ...
 const commands = {
	...
	randombattles(target, room, user) {
		...
		if (_optionalChain([room, 'optionalAccess', _9 => _9.battle, 'optionalAccess', _10 => _10.format, 'access', _11 => _11.includes, 'call', _12 => _12('nodmax')])) return this.parse(`/randombattlenodmax ${target}`);
		if (_optionalChain([room, 'optionalAccess', _13 => _13.battle, 'optionalAccess', _14 => _14.format, 'access', _15 => _15.includes, 'call', _16 => _16('doubles')])) return this.parse(`/randomdoublesbattle ${target}`);
 ...
 //# sourceMappingURL=sourceMaps/random-battles.js.map