"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; } var _class;var _lib = require('../../.lib-dist');
...
const filterWords = Chat.filterWords;
 ...
 const Filters = new (_class = class {
	 __init() {this.EVASION_DETECTION_SUB_STRINGS = {}}
	...
	load() {
		const legacy = _lib.FS.call(void 0, LEGACY_MONITOR_FILE);
		...
		const data = JSON.parse(_lib.FS.call(void 0, MONITOR_FILE).readIfExistsSync() || "{}");
		for (const k in data) {
			filterWords[k] = [];
	...
	loadLegacy() {
		let data;
		try {
			data = _lib.FS.call(void 0, LEGACY_MONITOR_FILE).readSync();
		} catch (e) {
...
}, _class); exports.Filters = Filters;
 ...
 const commands = {
	...
	filter: {
		...
		test(target, room, user) {
			...
			let [monitorName, message] = _lib.Utils.splitFirst(target, " ");
			...
			if (!(monitorName && message) || !_optionalChain([Chat, 'access', _15 => _15.monitors, 'access', _16 => _16[monitorName], 'optionalAccess', _17 => _17.monitor])) {
				return this.run((Chat.commands.filter ).testhelp);
			}
