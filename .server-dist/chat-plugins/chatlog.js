...
/** Match with two lines of context in either direction */
 ...
 class Searcher {constructor() { Searcher.prototype.__init.call(this); }
	__init() {this.roomstatsCache = new Map()}
	...
	async roomStats(room, month) {
		if (!_lib.FS.call(void 0, `logs/chat/${room}`).existsSync()) {
			return exports.LogViewer.error(_lib.Utils.html`Room ${room} not found.`);
		}
		if (!_lib.FS.call(void 0, `logs/chat/${room}/${month}`).existsSync()) {
			return exports.LogViewer.error(_lib.Utils.html`Room ${room} does not have logs for the month ${month}.`);
		}
		const stats = await exports.PM.query({
		...
		const next = exports.LogReader.nextMonth(month);
		const prev = exports.LogReader.prevMonth(month);
		const prevExists = _lib.FS.call(void 0, `logs/chat/${room}/${prev}`).existsSync();
		const nextExists = _lib.FS.call(void 0, `logs/chat/${room}/${next}`).existsSync();
		if (prevExists) {
	...
	visualizeStats(stats) {
		...
		for (const k in titles) {
			...
			switch (k) {
			case 'deadTime':
				buf += _chat.Chat.toDurationString(stats.deadTime, {precision: 2});
				break;
	...
	async activityStats(room, month) {
		const days = (await _lib.FS.call(void 0, `logs/chat/${room}/${month}`).readdir()).map(f => f.slice(0, -4));
		const stats = [];
		for (const day of days) {
	...
	async dayStats(room, day) {
		...
		const path = _lib.FS.call(void 0, `logs/chat/${room}/${exports.LogReader.getMonth(day)}/${day}.txt`);
		...
		for await (const line of stream.byLine()) {
			...
			case 'J': case 'j': {
				if (_optionalChain([rest, 'access', _3 => _3[0], 'optionalAccess', _4 => _4.startsWith, 'call', _5 => _5('*')])) continue; // ignore bots
 ...
 class RipgrepLogSearcher extends Searcher {
	async ripgrepSearchMonth(opts) {
		...
		const resultSep = _optionalChain([args, 'optionalAccess', _6 => _6.includes, 'call', _7 => _7('-m')]) ? '--' : '\n';
		try {
			const options = [
				'-e', search,
	...
	async searchLogs(
		...
		const user = _optionalChain([userRegex, 'access', _8 => _8.exec, 'call', _9 => _9(search), 'optionalAccess', _10 => _10[0], 'optionalAccess', _11 => _11.slice, 'call', _12 => _12(5)]);
	...
	async searchLinecounts(room, month, user) {
		...
		for (const fullLine of rawResults) {
			...
			if (user) {
				...
				const parts = _optionalChain([line, 'optionalAccess', _13 => _13.split, 'call', _14 => _14('|'), 'access', _15 => _15.map, 'call', _16 => _16(toID)]);
 ...
 const PM = new _lib.ProcessManager.QueryProcessManager(module, async data => {
	...
	try {
		...
		case 'battlesearch':
			...
			break;
		case 'roomstats':
			result = await exports.LogSearcher.activityStats(roomid, search);
			break;
		default:
		...
		if (_optionalChain([e, 'access', _17 => _17.name, 'optionalAccess', _18 => _18.endsWith, 'call', _19 => _19('ErrorMessage')])) {
...
;
		...
		if (!roomid || roomid.startsWith('-')) {
			this.title = '[Logs]';
			return exports.LogViewer.list(user, _optionalChain([roomid, 'optionalAccess', _20 => _20.slice, 'call', _21 => _21(1)]));
		}
		...
		let limit = null;
		let search;
		if (_optionalChain([opts, 'optionalAccess', _22 => _22.startsWith, 'call', _23 => _23('search-')])) {
	...
	roominfo(query, user) {
		this.checkCan('rangeban');
		const args = _lib.Utils.splitFirst(query.join('-'), '--', 2);
 ...
 const commands = {
	...
	searchlogs(target, room) {
		...
		const searches = [];
		let limit = '500';
		let targetRoom = _optionalChain([room, 'optionalAccess', _24 => _24.roomid]);
	...
	async sharedloggedbattles(target, room, user) {
		...
		if (_optionalChain([room, 'optionalAccess', _25 => _25.settings, 'access', _26 => _26.staffRoom]) || _optionalChain([this, 'access', _27 => _27.pmTarget, 'optionalAccess', _28 => _28.isStaff])) {
			this.runBroadcast();
		}
