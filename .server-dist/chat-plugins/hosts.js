...
function checkCanPerform(
	context, user, permission = 'lockdown'
) {
...
function getHostType(type) {
 ...
 function visualizeRangeList(ranges) {
	...
	for (const range of ranges) {
		...
		html += _lib.Utils.html`<td>${range.host}</td>`;
		html += `</tr>`;
	}
	return html;
...
function formatRange(range, includeModlogBrackets) {
 ...
 const pages = {
	proxies(query, user) {
		...
		_lib.Utils.sortBy(openProxies, ip => {
	...
	sharedipblacklist(args, user, connection) {
		...
		if (!Punishments.sharedIpBlacklist.size) {
			...
			_lib.Utils.sortBy(sortedSharedIPBlacklist, ([ipOrRange]) => {
	...
	sharedips(args, user, connection) {
		...
		if (!Punishments.sharedIps.size) {
			...
			_lib.Utils.sortBy(sortedSharedIPs, ([ip]) => {
				const number = IPTools.ipToNumber(ip);
				if (number === null) {
 ...
 //# sourceMappingURL=sourceMaps/hosts.js.map