...
var _roomsettings = require('./room-settings');
 ...
 */
 function runPromote(
	...
	if (_optionalChain([targetUser, 'optionalAccess', _ => _.locked]) && room.persist && room.settings.isPrivate !== true && nextGroup.rank > 2) {
 ...
 function runCrisisDemote(userid) {
	...
	if (section) {
		from.push(`Section Leader (${_roomsettings.RoomSections.sectionNames[section] || section})`);
...
Punishments.addPunishmentType('YEARLOCK', "Locked for a year", (user, punishment) => {
 ...
 const commands = {
	roomowner(target, room, user) {
		...
		if (room.settings.isPrivate === true) {
			...
			_optionalChain([Rooms, 'access', _2 => _2.get, 'call', _3 => _3(`upperstaff`), 'optionalAccess', _4 => _4.addByUser, 'call', _5 => _5(user, `<<${room.roomid}>> ${message}`), 'access', _6 => _6.update, 'call', _7 => _7()]);
		} else {
	...
	roompromote(target, room, user, connection, cmd) {
		...
		for (const toPromote of users) {
			...
			const targetUser = Users.getExact(userid);
			const name = _optionalChain([targetUser, 'optionalAccess', _11 => _11.name]) || toPromote;
			...
			if (this.pmTarget && targetUser) {
				...
				_optionalChain([shouldPopup, 'optionalAccess', _12 => _12.popup, 'call', _13 => _13(`You were demoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			} else if (nextSymbol === '#') {
				...
				_optionalChain([logRoom, 'optionalAccess', _14 => _14.addByUser, 'call', _15 => _15(user, `<<${room.roomid}>> ${name} was appointed Room Owner by ${user.name}`)]);
				this.modlog('ROOM OWNER', userid);
				_optionalChain([shouldPopup, 'optionalAccess', _16 => _16.popup, 'call', _17 => _17(`You were promoted to ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			} else {
				this.addModAction(`${name} was promoted to Room ${nextGroupName} by ${user.name}.`);
				this.modlog(`ROOM${nextGroupName.toUpperCase()}`, userid);
				_optionalChain([shouldPopup, 'optionalAccess', _18 => _18.popup, 'call', _19 => _19(`You were promoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`)]);
			}
	...
	authority(target, room, user, connection) {
		if (target && target !== '+') {
			...
			const availableRoom = _optionalChain([targetRoom, 'optionalAccess', _20 => _20.checkModjoin, 'call', _21 => _21(user)]);
		...
		const buffer = _lib.Utils.sortBy(
			...
			([symbol, names]) => (
				...
				_lib.Utils.sortBy(names, name => toID(name)).join(", ")
			)
		);
	...
	roomauth(target, room, user, connection, cmd) {
		...
		if (!_optionalChain([targetRoom, 'optionalAccess', _22 => _22.checkModjoin, 'call', _23 => _23(user)])) {
		...
		const buffer = _lib.Utils.sortBy(
			...
			return `${group}:\n` +
				_lib.Utils.sortBy(names).map(userid => {
					const isOnline = _optionalChain([Users, 'access', _24 => _24.get, 'call', _25 => _25(userid), 'optionalAccess', _26 => _26.statusType]) === 'online';
	...
	userauth(target, room, user, connection) {
		...
		const targetUsername = _optionalChain([targetUser, 'optionalAccess', _27 => _27.name]) || target;
		...
		if (sectionLeader) {
			buffer.push(`Section leader: ${_roomsettings.RoomSections.sectionNames[sectionLeader]}`);
		}
	...
	sectionleaders(target, room, user, connection) {
		...
		for (sectionid in sections) {
			...
			buffer.push(`**${_roomsettings.RoomSections.sectionNames[sectionid]}**:\n` + _lib.Utils.sortBy([...sections[sectionid]]).join(', '));
		}
	...
	part(target, room, user, connection) {
		...
		if (!targetRoom) {
			if (target.startsWith('view-')) {
				_optionalChain([connection, 'access', _28 => _28.openPages, 'optionalAccess', _29 => _29.delete, 'call', _30 => _30(target.slice(5))]);
				if (!_optionalChain([connection, 'access', _31 => _31.openPages, 'optionalAccess', _32 => _32.size])) connection.openPages = null;
	...
	warn(target, room, user) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _33 => _33.connected])) {
	...
	redir(target, room, user, connection) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _34 => _34.connected])) {
	...
	unmute(target, room, user) {
		...
		const successfullyUnmuted = room.unmute(
			_optionalChain([targetUser, 'optionalAccess', _35 => _35.id]) || toID(targetUsername), `Your mute in '${room.title}' has been lifted.`
		);
	...
	ban(target, room, user, connection, cmd) {
		...
		if (targetUser.id in room.users || user.can('lock')) {
			targetUser.popup(
				`|modal||html|<p>${_lib.Utils.escapeHTML(user.name)} has banned you from the room ${room.roomid} ` +
				`${(room.subRooms ? ` and its subrooms` : ``)}${week ? ' for a week' : ''}.` +
				`</p>${(publicReason ? `<p>Reason: ${_lib.Utils.escapeHTML(publicReason)}</p>` : ``)}` +
	...
	async lock(target, room, user, connection, cmd) {
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _36 => _36.battle])) this.parse('/savereplay forpunishment');
		return true;
	},
	lockhelp: [
	...
	unlock(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _37 => _37.namelocked])) {
			return this.errorReply(`User ${targetUser.name} is namelocked, not locked. Use /unnamelock to unnamelock them.`);
		}
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _38 => _38.locked]) && targetUser.locked.startsWith('#')) {
	...
	unlockip(target, room, user) {
		...
		for (const curUser of Users.findUsers([], [target])) {
			if (
				(range ? curUser.locked === '#rangelock' : !_optionalChain([curUser, 'access', _39 => _39.locked, 'optionalAccess', _40 => _40.startsWith, 'call', _41 => _41('#')])) &&
	...
	async globalban(target, room, user, connection, cmd) {
		...
		if (Users.isTrusted(userid)) {
			if (force) {
				const from = runCrisisDemote(userid);
				Monitor.log(`[CrisisMonitor] ${name} was globally banned by ${user.name} and demoted from ${_optionalChain([from, 'optionalAccess', _42 => _42.join, 'call', _43 => _43(", ")])}.`);
				this.globalModlog("CRISISDEMOTE", targetUser, ` from ${_optionalChain([from, 'optionalAccess', _44 => _44.join, 'call', _45 => _45(", ")])}`);
			} else {
		...
		const {privateReason, publicReason} = this.parseSpoiler(reason);
		_optionalChain([targetUser, 'optionalAccess', _46 => _46.popup, 'call', _47 => _47(
		...
		_optionalChain([room, 'optionalAccess', _48 => _48.hideText, 'call', _49 => _49([
			...affected.map(u => u.id),
			toID(inputUsername),
		])]);
	...
	banip(target, room, user) {
		...
		if (_optionalChain([curPunishment, 'optionalAccess', _50 => _50.type]) === 'BAN') {
	...
	yearlockname(target, room, user) {
		this.checkCan('rangeban');
		const [targetUsername, rest] = _lib.Utils.splitFirst(target, ',').map(k => k.trim());
		...
		if (_optionalChain([targetUser, 'optionalAccess', _51 => _51.registered]) && !force) {
	...
	promote(target, room, user, connection, cmd) {
		...
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _52 => _52.name]) || targetUsername;

		if (!userid) return this.parse('/help promote');

		const currentGroup = _optionalChain([targetUser, 'optionalAccess', _53 => _53.tempGroup]) || Users.globalAuth.get(userid);
	...
	trustuser(target, room, user, connection, cmd) {
		...
		if (rest) return this.errorReply(`This command does not support specifying a reason.`);
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _54 => _54.name]) || targetUsername;
	...
	sectionleader(target, room, user, connection, cmd) {
		...
		if (Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _55 => _55.id]) || userid) && !demoting) {
			throw new Chat.ErrorMessage(`${name} is already a Section Leader of ${_roomsettings.RoomSections.sectionNames[section]}.`);
		} else if (!Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _56 => _56.id]) || userid) && demoting) {
			throw new Chat.ErrorMessage(`${name} is not a Section Leader.`);
		}
		const staffRoom = Rooms.get('staff');
		if (!demoting) {
			Users.globalAuth.setSection(userid, section);
			this.addGlobalModAction(`${name} was appointed Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`);
			...
			_optionalChain([targetUser, 'optionalAccess', _57 => _57.popup, 'call', _58 => _58(`You were appointed Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		} else {
			const group = Users.globalAuth.get(userid);
			Users.globalAuth.deleteSection(userid);
			this.privateGlobalModAction(`${name} was demoted from Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`);
			...
			if (_optionalChain([staffRoom, 'optionalAccess', _59 => _59.auth, 'access', _60 => _60.getDirect, 'call', _61 => _61(userid)])  === '\u25B8') this.parse(`/msgroom staff,/roomdeauth ${userid}`);
			_optionalChain([targetUser, 'optionalAccess', _62 => _62.popup, 'call', _63 => _63(`You were demoted from Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		}
	...
	sectionleaderhelp: [
		...
		`Valid sections: ${_roomsettings.RoomSections.sections.join(', ')}`,
	...
	declare(target, room, user) {
		...
		this.add(_lib.Utils.html`|raw|<div class="broadcast-blue"><b>${target}</b></div>`);
	...
	htmldeclare(target, room, user) {
		...
		for (const u in room.users) {
			_optionalChain([Users, 'access', _64 => _64.get, 'call', _65 => _65(u), 'optionalAccess', _66 => _66.sendTo, 'call', _67 => _67(
				room,
				`|notify|${room.title} announcement!|${_lib.Utils.stripHTML(target)}`
			)]);
		}
	...
	notifyuser(target, room, user, connection, cmd) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _68 => _68.connected])) return this.errorReply(`User '${targetUsername}' not found.`);
	...
	forcerename(target, room, user) {
		...
		const targetID = _optionalChain([targetUser, 'optionalAccess', _69 => _69.id]) || toID(targetUsername);
		...
		if (_optionalChain([targetUser, 'optionalAccess', _70 => _70.connected])) {
		...
		if (_optionalChain([room, 'optionalAccess', _71 => _71.roomid]) !== 'staff') {
			if (_optionalChain([room, 'optionalAccess', _72 => _72.roomid, 'access', _73 => _73.startsWith, 'call', _74 => _74('help-')])) {
				this.addModAction(`${_optionalChain([targetUser, 'optionalAccess', _75 => _75.name]) || targetID} ${forceRenameMessage}`);
			} else {
				this.privateModAction(`${_optionalChain([targetUser, 'optionalAccess', _76 => _76.name]) || targetID} ${forceRenameMessage}`);
			}
		}
		const roomMessage = this.pmTarget ? `<PM:${this.pmTarget.id}>` :
			room && room.roomid !== 'staff' ? `«<a href="/${room.roomid}" target="_blank">${room.roomid}</a>» ` :
			'';
		const rankMessage = _optionalChain([targetUser, 'optionalAccess', _77 => _77.getAccountStatusString, 'call', _78 => _78()]) || "";
		Rooms.global.notifyRooms(
			['staff'],
			`|html|${roomMessage}` + _lib.Utils.html`<span class="username">${_optionalChain([targetUser, 'optionalAccess', _79 => _79.name]) || targetID}</span> ${rankMessage} ${forceRenameMessage}`
		);

		_optionalChain([targetUser, 'optionalAccess', _80 => _80.resetName, 'call', _81 => _81(true)]);
	...
	async namelock(target, room, user, connection, cmd) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _82 => _82.namelocked]) && !week) {
			return this.errorReply(`User '${targetUser.name}' is already namelocked.`);
		}
		const {privateReason, publicReason} = this.parseSpoiler(reason);
		const reasonText = publicReason ? ` (${publicReason})` : `.`;
		this.privateGlobalModAction(`${_optionalChain([targetUser, 'optionalAccess', _83 => _83.name]) || userid} was ${week ? 'week' : ''}namelocked by ${user.name}${reasonText}`);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _84 => _84.battle])) this.parse('/savereplay forpunishment');
	...
	unnamelock(target, room, user) {
		...
		const targetUser = Users.get(target);
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _85 => _85.namelocked])) {
	...
	hidetext(target, room, user, connection, cmd) {
		...
		if (/^[0-9]+\s*(,|$)/.test(reason)) {
			if (hasLineCount) {
				let lineCountString;
				[lineCountString, reason] = _lib.Utils.splitFirst(reason, ',').map(p => p.trim());
	...
	blacklist(target, room, user, connection, cmd) {
		...
		if (targetUser.id in room.users || user.can('lock')) {
			targetUser.popup(
				`|modal||html|<p>${_lib.Utils.escapeHTML(user.name)} has blacklisted you from the room ${room.roomid}${(room.subRooms ? ` and its subrooms` : '')}. Reason: ${_lib.Utils.escapeHTML(reason)}</p>` +
	...
	async groupchatban(target, room, user, connection, cmd) {
		...
		for (const roomid of createdGroupchats) {
			...
			const participants = _optionalChain([targetRoom, 'access', _86 => _86.warnParticipants, 'optionalCall', _87 => _87(
	...
	showblacklist(target, room, user, connection, cmd) {
		...
		let buf = _lib.Utils.html`Blacklist for ${room.title}${soonExpiring ? ` (expiring within 3 months)` : ''}:<br />`;
 ...
 //# sourceMappingURL=sourceMaps/moderation.js.map