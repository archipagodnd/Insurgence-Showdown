...
const SLOWCHAT_USER_REQUIREMENT = 10;
...
}; exports.RoomSections = RoomSections;
 ...
 const commands = {
	...
	modchat(target, room, user) {
		...
		if (_optionalChain([room, 'access', _ => _.battle, 'optionalAccess', _2 => _2.forcedSettings, 'access', _3 => _3.modchat]) && !user.can('rangeban')) {
	...
	inviteonlynext(target, room, user) {
		...
		if (!_optionalChain([groupConfig, 'optionalAccess', _4 => _4.editprivacy])) return this.errorReply(`/ionext - Access denied.`);
	...
	modjoin(target, room, user) {
		...
		if (target === 'sync' && !room.settings.modchat) {
			const lowestStaffGroup = Config.groupsranking.filter(group => _optionalChain([Config, 'access', _5 => _5.groups, 'access', _6 => _6[group], 'optionalAccess', _7 => _7.mute]))[0];
	...
	permissions: {
		...
		set(target, room, user) {
			...
			if (
				Users.Auth.ROOM_PERMISSIONS.includes(perm ) &&
		...
		''(target, room, user) {
			...
			const permissions = allPermissions.filter(perm => {
				const handler = _optionalChain([Chat, 'access', _8 => _8.parseCommand, 'call', _9 => _9(perm), 'optionalAccess', _10 => _10.handler]);
				if (_optionalChain([handler, 'optionalAccess', _11 => _11.isPrivate]) && !user.can('lock')) return false;
			...
			const subPermissions = allPermissions.filter(perm => perm.startsWith('/') && perm.includes(' ')).filter(perm => {
				const handler = _optionalChain([Chat, 'access', _12 => _12.parseCommand, 'call', _13 => _13(perm), 'optionalAccess', _14 => _14.handler]);
				if (_optionalChain([handler, 'optionalAccess', _15 => _15.isPrivate]) && !user.can('lock')) return false;
				return perm.startsWith('/') && perm.includes(' ');
			});
	...
	banword: {
		...
		add(target, room, user, connection, cmd) {
			...
			if (!regex) {
				words = words.map(word => {
					...
					return _lib.Utils.escapeRegex(word);
				});
			}
		...
		list(target, room, user) {
			...
			if (!_optionalChain([room, 'access', _16 => _16.settings, 'access', _17 => _17.banwords, 'optionalAccess', _18 => _18.length])) {
	...
	privateroom(target, room, user, connection, cmd) {
		...
		if (room.battle) {
			...
			if (_optionalChain([room, 'access', _19 => _19.tour, 'optionalAccess', _20 => _20.forcePublic])) {
	...
	hidenext(target, room, user) {
		const groupConfig = Config.groups[Users.PLAYER_SYMBOL];
		if (!_optionalChain([groupConfig, 'optionalAccess', _21 => _21.editprivacy])) return this.errorReply(`/hidenext - Access denied.`);
		if (this.meansNo(target)) {
	...
	roomsection(target, room, user) {
		...
		if (!target) {
			if (!this.runBroadcast()) return;
			this.sendReplyBox(_lib.Utils.html`This room is ${room.settings.section ? `in the ${sectionNames[room.settings.section]} section` : `not in a section`}.`);
			return;
		}
 ...
 const roomSettings = [
	...
	room => ({
		...
		permission: 'editroom',
		options: [
			[`tiers`, (_nullishCoalesce(room.settings.dataCommandTierDisplay, () => ( 'tiers'))) === 'tiers' || `roomtierdisplay tiers`],
 ...
 //# sourceMappingURL=sourceMaps/room-settings.js.map