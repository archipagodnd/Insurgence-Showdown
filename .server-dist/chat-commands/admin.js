...
async function updateserver(context, codePath) {
 ...
 const commands = {
	...
	addhtmlbox(target, room, user, connection, cmd) {
		...
		target = Chat.collapseLineBreaksHTML(target);
		if (user.tempGroup !== '*') {
			target += _lib.Utils.html`<div style="float:right;color:#888;font-size:8pt">[${user.name}]</div><div style="clear:both"></div>`;
		}
	...
	addrankhtmlbox(target, room, user, connection, cmd) {
		...
		if (user.tempGroup !== '*') {
			html += _lib.Utils.html`<div style="float:right;color:#888;font-size:8pt">[${user.name}]</div><div style="clear:both"></div>`;
		}
	...
	highlighthtmlpage(target, room, user) {
		const {targetUser, rest} = this.requireUser(target);
		let [pageid, title, highlight] = _lib.Utils.splitFirst(rest, ',', 2);
	...
	sendprivatehtmlbox(target, room, user, connection, cmd) {
		...
		html = `${_lib.Utils.html`<div style="color:#888;font-size:8pt">[Private from ${user.name}]</div>`}${Chat.collapseLineBreaksHTML(html)}`;
	...
	async hotpatch(target, room, user, connection, cmd) {
		...
		target = toID(target);
		try {
			_lib.Utils.clearRequireCache({exclude: ['/lib/process-manager']});
			if (target === 'all') {
				if (lock['all']) {
				...
				const processManagers = _lib.ProcessManager.processManagers;
				for (const manager of processManagers.slice()) {
					if (manager.filename.startsWith(_lib.FS.call(void 0, 'server/chat-plugins').path)) {
				...
				_lib.Utils.clearRequireCache();
				const newPM = require('../../.lib-dist/process-manager');
				require.cache = cache;

				const protos = [
					[_lib.ProcessManager.QueryProcessManager, newPM.QueryProcessManager],
					[_lib.ProcessManager.StreamProcessManager, newPM.StreamProcessManager],
					[_lib.ProcessManager.ProcessManager, newPM.ProcessManager],
					[_lib.ProcessManager.RawProcessManager, newPM.RawProcessManager],
					[_lib.ProcessManager.QueryProcessWrapper, newPM.QueryProcessWrapper],
					[_lib.ProcessManager.StreamProcessWrapper, newPM.StreamProcessWrapper],
					[_lib.ProcessManager.RawProcessManager, newPM.RawProcessWrapper],
				...
				void _optionalChain([Chat, 'access', _2 => _2.plugins, 'access', _3 => _3.datasearch, 'optionalAccess', _4 => _4.PM, 'optionalAccess', _5 => _5.respawn, 'call', _6 => _6()]);
				// broadcast the new formats list to clients
				Rooms.global.sendAll(Rooms.global.formatListText);
				this.sendReply("DONE");
			} else if (target === 'loginserver') {
				this.sendReply("Hotpatching loginserver...");
				_lib.FS.call(void 0, 'config/custom.css').unwatch();
				...
				const processManagers = _lib.ProcessManager.processManagers;
				for (const manager of processManagers.slice()) {
					if (manager.filename.startsWith(_lib.FS.call(void 0, 'server/modlog').path)) void manager.destroy();
				}
	...
	async processes(target, room, user) {
		...
		for (const manager of _lib.ProcessManager.processManagers) {
	...
	async savelearnsets(target, room, user, connection) {
		...
		await _lib.FS.call(void 0, 'data/learnsets.js').write(`'use strict';\n\nexports.Learnsets = {\n` +
	...
	kill(target, room, user) {
		...
		if (!_optionalChain([logRoom, 'optionalAccess', _7 => _7.log, 'access', _8 => _8.roomlogStream])) return process.exit();
	...
	async eval(target, room, user, connection) {
		...
		const generateHTML = (direction, contents) => (
			`<table border="0" cellspacing="0" cellpadding="0"><tr><td valign="top">` +
				_lib.Utils.escapeHTML(direction).repeat(2) +
				`&nbsp;</td><td>${Chat.getReadmoreCodeBlock(contents)}</td></tr><table>`
		);
		this.sendReply(`|html|${generateHTML('>', target)}`);
		_optionalChain([logRoom, 'optionalAccess', _9 => _9.roomlog, 'call', _10 => _10(`>> ${target}`)]);
		let uhtmlId = null;
		try {
			/* eslint-disable no-eval, @typescript-eslint/no-unused-vars */
			const battle = _optionalChain([room, 'optionalAccess', _11 => _11.battle]);
			const me = user;
			let result = eval(target);
			/* eslint-enable no-eval, @typescript-eslint/no-unused-vars */

			if (_optionalChain([result, 'optionalAccess', _12 => _12.then])) {
				...
				this.update();
				result = `Promise -> ${_lib.Utils.visualize(await result)}`;
			...
			_optionalChain([logRoom, 'optionalAccess', _13 => _13.roomlog, 'call', _14 => _14(`<< ${result}`)]);
		} catch (e) {
			...
			_optionalChain([logRoom, 'optionalAccess', _15 => _15.roomlog, 'call', _16 => _16(`<< ${message}`)]);
		}
	},
 ...
 //# sourceMappingURL=sourceMaps/admin.js.map