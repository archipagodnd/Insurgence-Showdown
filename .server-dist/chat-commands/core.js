...
for (const avatar of avatarTableGnomowladny) avatarTable.add(avatar);
 ...
 const crqHandlers = {
	userdetails(target, user, trustable) {
		...
		if (targetUser.locked) group = _nullishCoalesce(_optionalChain([Config, 'access', _ => _.punishgroups, 'optionalAccess', _2 => _2.locked, 'optionalAccess', _3 => _3.symbol]), () => ( '\u203d'));
		if (targetUser.namelocked) group = _nullishCoalesce(_optionalChain([Config, 'access', _4 => _4.punishgroups, 'optionalAccess', _5 => _5.namelocked, 'optionalAccess', _6 => _6.symbol]), () => ( '✖'));
...
}; exports.crqHandlers = crqHandlers;
 ...
 const commands = {
	...
	avatar(target, room, user) {
		...
		function customAvatarIsValid(userid) {
	...
	async showset(target, room, user, connection, cmd) {
		...
		let resultString = _lib.Utils.escapeHTML(Teams.export(team, {hideStats}));
		if (showAll) {
	...
	async accept(target, room, user, connection) {
		...
		this.sendChatMessage(_lib.Utils.html`/nonotify ${user.name} accepted the challenge, starting &laquo;<a href="/${gameRoom.roomid}">${gameRoom.roomid}</a>&raquo;`);
		return true;
	},
