...
const OFFLINE_SYMBOL = ` \u25CC `;
 ...
 const commands = {
	...
	whois(target, room, user, connection, cmd) {
		...
		if (Users.globalAuth.sectionLeaders.has(targetUser.id)) {
			buf += _lib.Utils.html`<br />Section Leader (${_roomsettings.RoomSections.sectionNames[Users.globalAuth.sectionLeaders.get(targetUser.id)]})`;
		}
		...
		if (canViewAlts) {
			let prevNames = targetUser.previousIDs.map(userid => {
				...
				return punishments.map(
					punishment => (
						`${userid}${punishment ? ` (${_optionalChain([Punishments, 'access', _17 => _17.punishmentTypes, 'access', _18 => _18.get, 'call', _19 => _19(punishment.type), 'optionalAccess', _20 => _20.desc]) || `punished`}` +
			...
			for (const targetAlt of targetUser.getAltUsers(true)) {
				...
				const punishMsg = !user.can('alts') ? '' : punishments.map(punishment => (
					` (${_optionalChain([Punishments, 'access', _21 => _21.punishmentTypes, 'access', _22 => _22.get, 'call', _23 => _23(punishment.type), 'optionalAccess', _24 => _24.desc]) || 'punished'}` +
				...
				prevNames = targetAlt.previousIDs.map(userid => {
					...
					return p.map(
						cur => `${userid} (${_optionalChain([Punishments, 'access', _25 => _25.punishmentTypes, 'access', _26 => _26.get, 'call', _27 => _27(cur.type), 'optionalAccess', _28 => _28.desc]) || 'punished'}` + `${cur.id !== targetAlt.id ? ` as ${cur.id}` : ``})`
					).join(' | ');
				}).join(", ");
		...
		if (canViewPunishments) {
			if (targetUser.namelocked) {
				...
				if (punishment) {
					...
					if (punishment.reason) buf += _lib.Utils.html` (reason: ${punishment.reason})`;
				}
			...
			if (battlebanned) {
				...
				if (battlebanned.reason) buf += _lib.Utils.html` (reason: ${battlebanned.reason})`;
			}
			...
			if (groupchatbanned) {
				...
				if (groupchatbanned.reason) buf += _lib.Utils.html` (reason: ${groupchatbanned.reason})`;
			}
			...
			if (ticketbanned) {
				...
				if (ticketbanned.reason) buf += _lib.Utils.html` (reason: ${ticketbanned.reason})`;
			}
			...
			const ips = targetUser.ips.map(ip => {
				...
				if (user.can('alts') && punishments) {
					for (const punishment of punishments) {
						...
						let punishMsg = _optionalChain([Punishments, 'access', _29 => _29.punishmentTypes, 'access', _30 => _30.get, 'call', _31 => _31(type), 'optionalAccess', _32 => _32.desc]) || type;
		...
		if (canViewPunishments) {
			...
			if (punishments.length) {
				...
				buf += punishments.map(([curRoom, curPunishment]) => {
					...
					let punishDesc = _optionalChain([Punishments, 'access', _33 => _33.roomPunishmentTypes, 'access', _34 => _34.get, 'call', _35 => _35(punishType), 'optionalAccess', _36 => _36.desc]) || punishType;
	...
	offlinewhois(target, room, user) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _37 => _37.connected])) buf += ` <em style="color:gray">(offline)</em>`;

		const roomauth = _optionalChain([room, 'optionalAccess', _38 => _38.auth, 'access', _39 => _39.getDirect, 'call', _40 => _40(userid)]);
		if (roomauth && _optionalChain([Config, 'access', _41 => _41.groups, 'access', _42 => _42[roomauth], 'optionalAccess', _43 => _43.name])) {
			buf += `<br />${Config.groups[roomauth].name} (${roomauth})`;
		}
		const group = Users.globalAuth.get(userid);
		if (_optionalChain([Config, 'access', _44 => _44.groups, 'access', _45 => _45[group], 'optionalAccess', _46 => _46.name])) {
			buf += `<br />Global ${Config.groups[group].name} (${group})`;
		}
		if (Users.globalAuth.sectionLeaders.has(userid)) {
			buf += `<br />Section Leader (${_roomsettings.RoomSections.sectionNames[Users.globalAuth.sectionLeaders.get(userid)]})`;
		}
		...
		if (idPunishments) {
			for (const p of idPunishments) {
				...
				const punishDesc = (_optionalChain([Punishments, 'access', _47 => _47.punishmentTypes, 'access', _48 => _48.get, 'call', _49 => _49(punishType), 'optionalAccess', _50 => _50.desc]) || punishType);
		...
		if (_optionalChain([punishments, 'optionalAccess', _51 => _51.length])) {
			buf += `<br />Room punishments: `;

			buf += punishments.map(([curRoom, curPunishment]) => {
				const {type: punishType, id: punishUserid, expireTime, reason} = curPunishment;
				let punishDesc = _optionalChain([Punishments, 'access', _52 => _52.roomPunishmentTypes, 'access', _53 => _53.get, 'call', _54 => _54(punishType), 'optionalAccess', _55 => _55.desc]) || punishType;
	...
	ipsearch(target, room, user, connection, cmd) {
		...
		if (/[a-z]/.test(ipOrHost)) {
			...
			for (const curUser of Users.users.values()) {
				...
				if (!_optionalChain([curUser, 'access', _56 => _56.latestHost, 'optionalAccess', _57 => _57.endsWith, 'call', _58 => _58(ipOrHost)])) continue;
	...
	checkchallenges(target, room, user) {
		...
		this.sendReplyBox(_lib.Utils.html`${from.name} is challenging ${to.name} in ${Dex.formats.get(chall.format).name}.`);
	},
	...
	data(target, room, user, connection, cmd) {
		...
		for (const [i, newTarget] of newTargets.entries()) {
			...
			case 'pokemon':
				...
				if (_optionalChain([format, 'optionalAccess', _59 => _59.onModifySpecies])) {
					pokemon = format.onModifySpecies.call({dex, clampIntRange: _lib.Utils.clampIntRange, toID} , pokemon) || pokemon;
				}
				let tierDisplay = _optionalChain([room, 'optionalAccess', _60 => _60.settings, 'access', _61 => _61.dataCommandTierDisplay]);
				if (!tierDisplay && _optionalChain([room, 'optionalAccess', _62 => _62.battle])) {
			...
			case 'move':
				...
				if (showDetails) {
					...
					if (dex.gen >= 7) {
						...
						} else if (_optionalChain([move, 'access', _63 => _63.zMove, 'optionalAccess', _64 => _64.basePower])) {
							details["Z-Power"] = String(move.zMove.basePower);
						} else if (_optionalChain([move, 'access', _65 => _65.zMove, 'optionalAccess', _66 => _66.effect])) {
							const zEffects = {
						...
						} else if (_optionalChain([move, 'access', _67 => _67.zMove, 'optionalAccess', _68 => _68.boost])) {
							details["Z-Effect"] = "";
					...
					if (dex.gen >= 8) {
						...
						} else if (_optionalChain([move, 'access', _69 => _69.maxMove, 'optionalAccess', _70 => _70.basePower])) {
	...
	weakness(target, room, user) {
		...
		if (species.exists) {
			...
			if (types.length === 0) {
				return this.sendReplyBox(_lib.Utils.html`${target} isn't a recognized type or Pokemon${Dex.gen > dex.gen ? ` in Gen ${dex.gen}` : ""}.`);
			}
			species = {types: types};
			target = types.join("/");
		}
		...
		if (dex.getImmunity(source, defender) ||
			...
			if (source.effectType !== 'Move' || source.category !== 'Status' && (source.basePower || source.basePowerCallback)) {
				for (const type of defender.types) {
					...
					const moveMod = _optionalChain([source, 'access', _71 => _71.onEffectiveness, 'optionalAccess', _72 => _72.call, 'call', _73 => _73({dex: Dex} , baseMod, null, type, source)]);
		...
		for (let arg of targets) {
			...
			for (const type in bestCoverage) {
				if (move.id === "struggle") {
					...
					const moveMod = _optionalChain([move, 'access', _74 => _74.onEffectiveness, 'optionalAccess', _75 => _75.call, 'call', _76 => _76({dex} , baseMod, null, type, move )]);
					...
					if (type1 === type2) {
						...
						for (const move of sources) {
							...
							if (typeof move === 'string') {
								...
								let moveMod = _optionalChain([move, 'access', _77 => _77.onEffectiveness, 'optionalAccess', _78 => _78.call, 'call', _79 => _79({dex} , baseMod, null, type1, move )]);
								curEff += typeof moveMod === 'number' ? moveMod : baseMod;
								baseMod = dex.getEffectiveness(move.type, type2);
								moveMod = _optionalChain([move, 'access', _80 => _80.onEffectiveness, 'optionalAccess', _81 => _81.call, 'call', _82 => _82({dex} , baseMod, null, type2, move )]);
								curEff += typeof moveMod === 'number' ? moveMod : baseMod;
							}
	...
	bugs(target, room, user) {
		if (!this.runBroadcast()) return;
		if (_optionalChain([room, 'optionalAccess', _83 => _83.battle])) {
	...
	calc(target, room, user, connection, cmd) {
		...
		const isRandomBattle = (_optionalChain([room, 'optionalAccess', _84 => _84.battle]) && SUPPORTED_RANDOM_FORMATS.includes(room.battle.format));
		const isBattleSpotBattle = (_optionalChain([room, 'optionalAccess', _85 => _85.battle]) && (SUPPORTED_BATTLESPOT_FORMATS.includes(room.battle.format) ||
	...
	formathelp(target, room, user, connection, cmd) {
		...
		if (format) {
			...
			if (['Format', 'Rule', 'ValidatorRule'].includes(format.effectType)) {
				if (_optionalChain([format, 'access', _86 => _86.ruleset, 'optionalAccess', _87 => _87.length])) {
					rules.push(`<b>Ruleset</b> - ${_lib.Utils.escapeHTML(format.ruleset.join(", "))}`);
				}
				if (_optionalChain([format, 'access', _88 => _88.banlist, 'optionalAccess', _89 => _89.length])) {
					rules.push(`<b>Bans</b> - ${_lib.Utils.escapeHTML(format.banlist.join(", "))}`);
				}
				if (_optionalChain([format, 'access', _90 => _90.unbanlist, 'optionalAccess', _91 => _91.length])) {
					rules.push(`<b>Unbans</b> - ${_lib.Utils.escapeHTML(format.unbanlist.join(", "))}`);
				}
				if (_optionalChain([format, 'access', _92 => _92.restricted, 'optionalAccess', _93 => _93.length])) {
					rules.push(`<b>Restricted</b> - ${_lib.Utils.escapeHTML(format.restricted.join(", "))}`);
				}
				if (rules.length > 0) {
	...
	rules(target, room, user, connection, cmd) {
		if (!target) {
			...
			this.sendReplyBox(
				...
				`${_optionalChain([room, 'optionalAccess', _94 => _94.settings, 'access', _95 => _95.rulesLink]) ? _lib.Utils.html`- <a href="${room.settings.rulesLink}">${this.tr`${room.title} room rules`}</a><br />` : ``}` +
	...
	smogdex(target, room, user) {
		...
		if (pokemon.exists) {
			...
			if (['ou', 'uu'].includes(formatId) && generation === 'sm' &&
				_optionalChain([room, 'optionalAccess', _96 => _96.settings, 'access', _97 => _97.language]) && room.settings.language in supportedLanguages) {
	...
	shuffle(target, room, user) {
		if (!_optionalChain([target, 'optionalAccess', _98 => _98.includes, 'call', _99 => _99(',')])) return this.parse('/help shuffle');
	...
	async requestshow(target, room, user) {
		...
		if (_optionalChain([room, 'access', _100 => _100.pendingApprovals, 'optionalAccess', _101 => _101.has, 'call', _102 => _102(user.id)])) return this.errorReply('You have a request pending already.');
	...
	async approveshow(target, room, user) {
		...
		const request = _optionalChain([room, 'access', _103 => _103.pendingApprovals, 'optionalAccess', _104 => _104.get, 'call', _105 => _105(userid)]);
		...
		if (request.dimensions) { // image
			...
			buf = await _youtube.YouTube.generateVideoDisplay(request.link);
	...
	denyshow(target, room, user) {
		...
		const entry = _optionalChain([room, 'access', _106 => _106.pendingApprovals, 'optionalAccess', _107 => _107.get, 'call', _108 => _108(target)]);
	...
	async show(target, room, user, connection) {
		if (!_optionalChain([room, 'optionalAccess', _109 => _109.persist]) && !this.pmTarget) return this.errorReply(`/show cannot be used in temporary rooms.`);
		...
		let buf;
		if (_youtube.YouTube.linkRegex.test(link)) {
			buf = await _youtube.YouTube.generateVideoDisplay(link);
			this.message = this.message.replace(/&ab_channel=(.*)(&|)/ig, '').replace(/https:\/\/www\./ig, '');
		} else if (_youtube.Twitch.linkRegex.test(link)) {
			const channelId = _optionalChain([_youtube.Twitch, 'access', _110 => _110.linkRegex, 'access', _111 => _111.exec, 'call', _112 => _112(link), 'optionalAccess', _113 => _113[2], 'optionalAccess', _114 => _114.trim, 'call', _115 => _115()]);
			if (!channelId) return this.errorReply(`Specify a Twitch channel.`);
			const info = await _youtube.Twitch.getChannel(channelId);
	...
	async registertime(target, room, user, connection) {
		...
		const regTimeAgo = duration > 24 * 60 * 60 * 1000 ?
			...
			'less than a day';
		this.sendReplyBox(_lib.Utils.html`The user '${target}' registered ${regTimeAgo} ago, on the date ${date.toDateString()}.`);
	},
	...
	code(target, room, user, connection) {
		...
		const isPMOrPersonalRoom = _optionalChain([this, 'access', _116 => _116.room, 'optionalAccess', _117 => _117.settings, 'access', _118 => _118.isPersonal]) !== false;
 ...
 const pages = {
	...
	punishments(query, user) {
		...
		this.checkCan('mute', null, room);
		// Ascending order
		const sortedPunishments = _lib.Utils.sortBy([...Punishments.getPunishments(room.roomid)], ([id, entry]) => (
			entry.expireTime
		));
		const sP = new Map();
	...
	globalpunishments(query, user) {
		...
		this.checkCan('lock');
		// Ascending order
		const sortedPunishments = _lib.Utils.sortBy([...Punishments.getPunishments()], ([id, entry]) => (
			entry.expireTime
		));
		const sP = new Map();
...
process.nextTick(() => {
 ...
 //# sourceMappingURL=sourceMaps/info.js.map