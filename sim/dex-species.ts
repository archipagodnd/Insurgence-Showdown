...
export class Species extends BasicEffect implements Readonly<BasicEffect & SpeciesFormatsData> {
	...
	constructor(data: AnyObject) {
		...
		this.isMega = !!(this.forme && ['Mega', 'Mega-X', 'Mega-Y', 'Mega-Fire', 'Mega-Steel'].includes(this.forme)) || undefined;
		...
		if (id && this.dex.data.Pokedex.hasOwnProperty(id)) {
			...
			if (this.dex.currentMod === 'letsgo' && !species.isNonstandard) {
				const isLetsGo = (
					...
					(!species.forme || ['Alola', 'Mega', 'Mega-X', 'Mega-Y', 'Mega-Fire', 'Mega-Steel', 'Starter'].includes(species.forme))
				);
