...
export class TeamValidator {
	...
	validateStats(set: PokemonSet, species: Species, setSources: PokemonSources) {
		...
		if (!this.format.debug) {
			...
			// Note that in VGC etc (Adjust Level Down = 50), `set.level` will be 120 here for validation purposes
	...
	validateEvent(
		...
		if (ruleTable.has('obtainableabilities')) {
			...
			if (species.abilities['H']) {
				...
				if (!isHidden && eventData.isHidden && !canUseDreamMist && dex.gen <= 8) {
					if (fastReturn) return true;
			...
			if (sources) {
				for (let learned of sources) {
					...
					if (learned.charAt(1) === 'L') {
						...
						if (
							level >= parseInt(learned.substr(2)) ||
							learnedGen === 7 ||
							(learnedGen === 6 && species.name.endsWith('Delta')) ||
							(learnedGen === 6 && species.name.slice(-7, -2) === 'Delta') &&
							dex.gen >= 7
							) {
