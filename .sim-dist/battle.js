...
var _lib = require('../.lib-dist');
 ...
 class Battle {
	...
	singleEvent(
		eventid, effect, state,
		target, source,
		sourceEffect, relayVar, customCallback
	) {
	...
	findEventHandlers(target, eventName, source) {
		...
		if (target instanceof _pokemon.Pokemon && (target.isActive || _optionalChain([source, 'optionalAccess', _ => _.isActive]))) {
	...
	getRequests(type) {
		...
		case 'switch':
			for (let i = 0; i < this.sides.length; i++) {
				...
				const switchTable = side.active.map(pokemon => !!_optionalChain([pokemon, 'optionalAccess', _2 => _2.switchFlag]));
		...
		default:
			for (let i = 0; i < this.sides.length; i++) {
				const side = this.sides[i];
				if (!side.pokemonLeft) continue;
				const activeData = side.active.map(pokemon => _optionalChain([pokemon, 'optionalAccess', _3 => _3.getMoveRequestData, 'call', _4 => _4()]));
	...
	win(side) {
		...
		this.add('');
		if (_optionalChain([side, 'optionalAccess', _5 => _5.allySide])) {
	...
	lose(side) {
		...
		side.pokemonLeft = 0;
		_optionalChain([side, 'access', _6 => _6.active, 'access', _7 => _7[0], 'optionalAccess', _8 => _8.faint, 'call', _9 => _9()]);
	...
	boost(
		...
		if (!_optionalChain([target, 'optionalAccess', _10 => _10.hp])) return 0;
		...
		for (boostName in boost) {
			...
			if (boostBy) {
				success = true;
				switch (_optionalChain([effect, 'optionalAccess', _11 => _11.id])) {
				case 'bellydrum':
	...
	directDamage(damage, target, source = null, effect = null) {
		...
		if (!_optionalChain([target, 'optionalAccess', _12 => _12.hp])) return 0;
		if (!damage) return 0;
		...
		if (this.gen <= 1 && this.dex.currentMod !== 'gen1stadium' &&
			...
			if (_optionalChain([source, 'optionalAccess', _13 => _13.volatiles, 'access', _14 => _14['substitute']])) {
	...
	heal(damage, target, source = null, effect = null) {
		...
		if (!_optionalChain([target, 'optionalAccess', _15 => _15.hp])) return false;
		if (!target.isActive) return false;
		if (target.hp >= target.maxhp) return false;
		const finalDamage = target.heal(damage, source, effect);
		switch (_optionalChain([effect, 'optionalAccess', _16 => _16.id])) {
		case 'leechseed':
		case 'rest':
	...
	getTarget(pokemon, move, targetLoc, originalTarget) {
		...
		if (move.target !== 'randomNormal' && this.validTargetLoc(targetLoc, pokemon, move.target)) {
			...
			if (_optionalChain([target, 'optionalAccess', _17 => _17.fainted])) {
				if (this.gameType === 'freeforall') {
	...
	runAction(action) {
		const pokemonOriginalHP = _optionalChain([action, 'access', _18 => _18.pokemon, 'optionalAccess', _19 => _19.hp]);
		let residualPokemon = [];
		...
		if (!this.queue.peek() || (this.gen <= 3 && ['move', 'residual'].includes(this.queue.peek().choice))) {
			...
			return false;
		} else if (_optionalChain([this, 'access', _20 => _20.queue, 'access', _21 => _21.peek, 'call', _22 => _22(), 'optionalAccess', _23 => _23.choice]) === 'instaswitch') {
			return false;
		}
		...
		if (this.gen >= 8 && _optionalChain([this, 'access', _24 => _24.queue, 'access', _25 => _25.peek, 'call', _26 => _26(), 'optionalAccess', _27 => _27.choice]) === 'move') {
