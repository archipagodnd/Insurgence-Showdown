 ...
 */
 ...
 class TeamValidator {
	...
	constructor(format, dex = _dex.Dex) {
 ...
 = {}
	...
	validateSet(set, teamHas) {
		...
		let item = dex.items.get(_lib.Utils.getString(set.item));
		set.item = item.name;
		let ability = dex.abilities.get(_lib.Utils.getString(set.ability));
		set.ability = ability.name;
		let nature = dex.natures.get(_lib.Utils.getString(set.nature));
		...
		if (ruleTable.has('obtainableformes')) {
			...
			} else if (canMegaEvo && species.id === 'rayquaza' && set.moves.map(_dex.toID).includes('dragonascent' )) {
		...
		for (const moveName of set.moves) {
			...
			const move = dex.moves.get(_lib.Utils.getString(moveName));
	...
	validateStats(set, species, setSources) {
		...
		if (dex.gen <= 2) {
			...
			if (
				set.species === 'Marowak' && _dex.toID.call(void 0, set.item) === 'thickclub' &&
				set.moves.map(_dex.toID).includes('swordsdance' ) && set.level === 100
			) {
				// Marowak hack
		...
		for (const stat in set.evs) {
			if (set.evs[stat ] < 0) {
				problems.push(`${name} has less than 0 ${allowAVs ? 'Awakening Values' : 'EVs'} in ${_dex.Dex.stats.names[stat ]}.`);
			}
		}
		...
		if (dex.currentMod === 'gen7letsgo') { // AVs
			for (const stat in set.evs) {
				if (set.evs[stat ] > 0 && !allowAVs) {
					...
					problems.push(`${name} has more than 200 Awakening Values in ${_dex.Dex.stats.names[stat ]}.`);
				}
			}
		} else { // EVs
			for (const stat in set.evs) {
				if (set.evs[stat ] > 255) {
					problems.push(`${name} has more than 255 EVs in ${_dex.Dex.stats.names[stat ]}.`);
				}
			}
			if (dex.gen <= 2) {
	 ...
	 */
	validateSource(
		set, source, setSources, species, because
	) {
		let eventData;
		...
		return this.validateEvent(set, eventData, eventSpecies, because ) ;
	}
	...
	validateForme(set) {
		...
		if (species.name === 'Necrozma-Ultra') {
			...
			if (species.requiredMove && !set.moves.map(_dex.toID).includes(_dex.toID.call(void 0, species.requiredMove))) {
		...
		if (species.name === 'Keldeo' && set.moves.map(_dex.toID).includes('secretsword' ) && dex.gen >= 8) {
		...
		if (set.species in crowned) {
			const ironHead = set.moves.map(_dex.toID).indexOf('ironhead' );
			if (ironHead >= 0) {
	...
	checkSpecies(set, species, tierSpecies, setHas) {
		...
		setHas['basepokemon:' + _dex.toID.call(void 0, species.baseSpecies)] = true;
		...
		const tierTag = 'pokemontag:' + _dex.toID.call(void 0, tier);
		setHas[tierTag] = true;

		const doublesTier = tierSpecies.doublesTier === '(DUU)' ? 'DNU' : tierSpecies.doublesTier;
		const doublesTierTag = 'pokemontag:' + _dex.toID.call(void 0, doublesTier);
		...
		banReason = ruleTable.check('basepokemon:' + _dex.toID.call(void 0, species.baseSpecies));
		if (banReason) {
			return `${species.name} is ${banReason}.`;
		}
		if (banReason === '') {
		...
		let nonexistentCheck = _tags.Tags.nonexistent.genericFilter(tierSpecies) && ruleTable.check('nonexistent');
	...
	checkItem(set, item, setHas) {
		...
		if (item.isNonstandard) {
			banReason = ruleTable.check('pokemontag:' + _dex.toID.call(void 0, item.isNonstandard));
			if (banReason) {
	...
	checkMove(set, move, setHas) {
		...
		if (move.isNonstandard) {
			banReason = ruleTable.check('pokemontag:' + _dex.toID.call(void 0, move.isNonstandard));
			if (banReason) {
	...
	checkAbility(set, ability, setHas) {
		...
		if (ability.isNonstandard) {
			banReason = ruleTable.check('pokemontag:' + _dex.toID.call(void 0, ability.isNonstandard));
			if (banReason) {
	...
	checkNature(set, nature, setHas) {
		...
		if (nature.isNonstandard) {
			banReason = ruleTable.check('pokemontag:' + _dex.toID.call(void 0, nature.isNonstandard));
			if (banReason) {
	 ...
	 * If `because` is not passed, instead returns true if invalid.
	 */
	validateEvent(set, eventData, eventSpecies, because = ``, from = `from an event`) {
		const dex = this.dex;
		let name = set.species;
		const species = dex.species.get(set.species);
		...
		if (eventData.ivs) {
			...
			for (statName in eventData.ivs) {
				...
				if (set.ivs[statName] !== eventData.ivs[statName]) {
					if (fastReturn) return true;
					problems.push(`${name} must have ${eventData.ivs[statName]} ${_dex.Dex.stats.names[statName]} IVs${etc}.`);
				}
			}
			...
			if (dex.gen <= 5 && eventData.abilities && eventData.abilities.length === 1 && !eventData.isHidden) {
				if (species.name === eventSpecies.name) {
					...
					if (ability1.gen && eventData.generation >= ability1.gen) {
						...
						const requiredAbilitySlot = (_dex.toID.call(void 0, eventData.abilities[0]) === ability1.id ? 1 : 0);
	...
	validateMoves(
		...
		if (setSources.babyOnly && setSources.sources.length) {
			...
			const babyEvo = _dex.toID.call(void 0, baby.evos[0]);
 ...
 //# sourceMappingURL=sourceMaps/team-validator.js.map