...
// there are very few non-malicious competitive reasons to do so
...
] ); exports.RESTORATIVE_BERRIES = RESTORATIVE_BERRIES;
 ...
 class Pokemon {
	...
	constructor(set, side) {;Pokemon.prototype.__init.call(this);Pokemon.prototype.__init2.call(this);
		...
		this.baseMoveSlots = [];
		this.moveSlots = [];
		if (!_optionalChain([this, 'access', _ => _.set, 'access', _2 => _2.moves, 'optionalAccess', _3 => _3.length])) {
	...
	getStat(statName, unboosted, unmodified) {
		...
		if (statName === 'spe' && stat > 10000 && !_optionalChain([this, 'access', _4 => _4.battle, 'access', _5 => _5.format, 'access', _6 => _6.battle, 'optionalAccess', _7 => _7.trunc])) stat = 10000;
		return stat;
	}
	...
	*/
	...
	getLastDamagedBy(filterOutSameSide) {
		const damagedBy = this.attackedBy.filter(attacker => (
...

 {
	...
	maxMoveDisabled(baseMove) {
		...
		if (!_optionalChain([this, 'access', _8 => _8.getMoveData, 'call', _9 => _9(baseMove.id), 'optionalAccess', _10 => _10.pp])) return true;
	...
	getDynamaxRequest(skipChecks) {
		...
		if (!skipChecks) {
			...
			if (this.species.cannotDynamax || _optionalChain([this, 'access', _11 => _11.illusion, 'optionalAccess', _12 => _12.species, 'access', _13 => _13.cannotDynamax])) return;
		}
 ...
 = {
	...
	transformInto(pokemon, effect) {
		...
		if (this.battle.gen === 4) {
			...
			if (this.species.num === 493) {
				...
				const targetForme = (_optionalChain([item, 'optionalAccess', _14 => _14.onPlate]) ? 'Arceus-' + item.onPlate : 'Arceus');
	 ...
	 */
	formeChange(
		speciesId, source = this.battle.effect,
		isPermanent, message
	) {
	...
	disableMove(moveid, isHidden, sourceEffect) {
		...
		for (const moveSlot of this.moveSlots) {
			if (moveSlot.id === moveid && moveSlot.disabled !== true) {
				...
				moveSlot.disabledSource = (_optionalChain([sourceEffect, 'optionalAccess', _15 => _15.fullname]) || moveSlot.move);
			}
		}
	}
	...
	setStatus(
		...
		if (this.status === status.id) {
			if (_optionalChain([(sourceEffect ), 'optionalAccess', _16 => _16.status]) === this.status) {
				this.battle.add('-fail', this, this.status);
			} else if (_optionalChain([(sourceEffect ), 'optionalAccess', _17 => _17.status])) {
		...
		if (!ignoreImmunities && status.id &&
				!(_optionalChain([source, 'optionalAccess', _18 => _18.hasAbility, 'call', _19 => _19('corrosion')]) && ['tox', 'psn'].includes(status.id))) {
			// the game currently never ignores immunities
			if (!this.runStatusImmunity(status.id === 'tox' ? 'psn' : status.id)) {
				this.battle.debug('immune to status');
				if (_optionalChain([(sourceEffect ), 'optionalAccess', _20 => _20.status])) {
	...
	addVolatile(
		...
		if (!this.runStatusImmunity(status.id)) {
			...
			if (_optionalChain([(sourceEffect ), 'optionalAccess', _21 => _21.status])) {
				this.battle.add('-immune', this);
			}
			return false;
		}
 ...
 //# sourceMappingURL=sourceMaps/pokemon.js.map