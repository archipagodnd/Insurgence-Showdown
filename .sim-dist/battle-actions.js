"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }var _dex = require('./dex');
...
const CHOOSABLE_TARGETS = new Set(['normal', 'any', 'adjacentAlly', 'adjacentAllyOrSelf', 'adjacentFoe']);
 ...
 class BattleActions {
	...
	switchIn(pokemon, pos, sourceEffect = null, isDrag) {
		...
		const unfaintedActive = _optionalChain([oldActive, 'optionalAccess', _ => _.hp]) ? oldActive : null;
		if (unfaintedActive) {
 ...
 = [
	...
	hitStepMoveHitLoop(targets, pokemon, move) { // Temporary name
		...
		for (hit = 1; hit <= targetHits; hit++) {
			...
			if (targets.every(target => !_optionalChain([target, 'optionalAccess', _2 => _2.hp]))) break;
			move.hit = hit;
	...
	tryPrimaryHitEvent(
		damage, targets, pokemon,
		move, moveData, isSecondary
	) {
		for (const [i, target] of targets.entries()) {
			if (!target) continue;
	...
	getSpreadDamage(
		damage, targets, source,
		move, moveData, isSecondary, isSelf
	) {
		for (const [i, target] of targets.entries()) {
			if (!target) continue;
			this.battle.activeTarget = target;
	...
	getZMove(move, pokemon, skipChecks) {
		...
		if (!skipChecks) {
			...
			if (!_optionalChain([moveData, 'optionalAccess', _3 => _3.pp])) return;
		}
		...
		if (item.zMoveFrom) {
			...
			if (move.type === item.zMoveType) {
				if (move.category === "Status") {
					return move.name;
				} else if (_optionalChain([move, 'access', _4 => _4.zMove, 'optionalAccess', _5 => _5.basePower])) {
	...
	getActiveMaxMove(move, pokemon) {
		...
		if (move.category !== 'Status') {
			...
			if (!_optionalChain([move, 'access', _6 => _6.maxMove, 'optionalAccess', _7 => _7.basePower])) throw new Error(`${move.name} doesn't have a maxMove basePower`);
	...
	runZPower(move, pokemon) {
		...
		} else if (_optionalChain([move, 'access', _10 => _10.zMove, 'optionalAccess', _11 => _11.effect])) {
...
(
	...
	canMegaEvo(pokemon) {
		...
		if ((this.battle.gen <= 7 || this.battle.ruleTable.has('standardnatdex')) &&
			_optionalChain([altForme, 'optionalAccess', _12 => _12.isMega]) && _optionalChain([altForme, 'optionalAccess', _13 => _13.requiredMove]) &&
			pokemon.baseMoves.includes(_dex.toID.call(void 0, altForme.requiredMove)) && !item.zMove) {
	...
	runMegaEvo(pokemon) {
		...
		for (const foeActive of pokemon.foes()) {
			if (_optionalChain([foeActive, 'access', _14 => _14.volatiles, 'access', _15 => _15['skydrop'], 'optionalAccess', _16 => _16.source]) === pokemon) {
				return false;
			}
		}
 ...
 //# sourceMappingURL=sourceMaps/battle-actions.js.map