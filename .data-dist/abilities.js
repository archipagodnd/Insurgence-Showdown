 ...
 const Abilities = {
	...
	ancientpresence: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _ => _.hasAbility, 'call', _2 => _2('wonderguard')])) return;
			...
			if (move.ignoreImmunity !== true) {
				...
				if (_optionalChain([target, 'optionalAccess', _3 => _3.hasAbility, 'call', _4 => _4('etherealshroud')]) === false) {
					move.ignoreImmunity['Fighting'] = true;
					move.ignoreImmunity['Normal'] = true;
				}
				if (
					_optionalChain([target, 'optionalAccess', _5 => _5.volatiles, 'access', _6 => _6['magnetrise']]) || _optionalChain([target, 'optionalAccess', _7 => _7.hasItem, 'call', _8 => _8('airballoon')]) ||
					_optionalChain([target, 'optionalAccess', _9 => _9.hasAbility, 'call', _10 => _10('levitate')]) || _optionalChain([target, 'optionalAccess', _11 => _11.hasAbility, 'call', _12 => _12('omnitype')])
				) {
	...
	angerpoint: {
		onHit(target, source, move) {
			if (!target.hp) return;
			if (_optionalChain([move, 'optionalAccess', _13 => _13.effectType]) === 'Move' && target.getMoveHitData(move).crit) {
	...
	battlebond: {
		onSourceAfterFaint(length, target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _14 => _14.effectType]) !== 'Move') {
				return;
			}
	...
	galewings: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _16 => _16.type]) === 'Flying' && pokemon.hp === pokemon.maxhp) return priority + 1;
		},
		name: "Gale Wings",
		rating: 3,
		num: 177,
	...
	insomnia: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _18 => _18.status])) {
	...
	irrelephant: {
		onModifyMovePriority: -5,
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _19 => _19.hasAbility, 'call', _20 => _20('wonderguard')])) return;
			if (!move.ignoreImmunity) move.ignoreImmunity = {};
			if (move.ignoreImmunity !== true) {
				move.ignoreImmunity['Psychic'] = true;
				move.ignoreImmunity['Electric'] = true;
				...
				move.ignoreImmunity['Ghost'] = true;
				move.ignoreImmunity['Dragon'] = true;
				if (_optionalChain([target, 'optionalAccess', _21 => _21.hasAbility, 'call', _22 => _22('etherealshroud')]) === false) {
					move.ignoreImmunity['Fighting'] = true;
					move.ignoreImmunity['Normal'] = true;
				}
				if (
					_optionalChain([target, 'optionalAccess', _23 => _23.volatiles, 'access', _24 => _24['magnetrise']]) || _optionalChain([target, 'optionalAccess', _25 => _25.hasItem, 'call', _26 => _26('airballoon')]) ||
					_optionalChain([target, 'optionalAccess', _27 => _27.hasAbility, 'call', _28 => _28('levitate')]) || _optionalChain([target, 'optionalAccess', _29 => _29.hasAbility, 'call', _30 => _30('omnitype')])
				) {
					move.ignoreImmunity['Ground'] = false;
				} else {
					move.ignoreImmunity['Ground'] = true;
	...
	leafguard: {
		onSetStatus(status, target, source, effect) {
			if (['sunnyday', 'desolateland'].includes(target.effectiveWeather())) {
				if (_optionalChain([(effect ), 'optionalAccess', _31 => _31.status])) {
	...
	limber: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _32 => _32.status])) {
	...
	naturalcure: {
		onCheckShow(pokemon) {
			...
			for (const curPoke of pokemon.side.active) {
				...
				if (!_optionalChain([curPoke, 'optionalAccess', _33 => _33.status])) {
	...
	omnitype: {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (_optionalChain([target, 'optionalAccess', _34 => _34.species, 'access', _35 => _35.id]) !== 'giratinaprimal') return;
			...
			if (_optionalChain([target, 'optionalAccess', _36 => _36.types, 'access', _37 => _37.length]) === 2) {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _38 => _38.status])) {
				this.add('-immune', target, '[from] ability: Omnitype');
			}
			return false;
		},
	...
	owntempo: {
		...
		onHit(target, source, move) {
			if (_optionalChain([move, 'optionalAccess', _39 => _39.volatileStatus]) === 'confusion') {
	...
	pastelveil: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _40 => _40.status])) {
		...
		onAllySetStatus(status, target, source, effect) {
			if (!['psn', 'tox'].includes(status.id)) return;
			if (_optionalChain([(effect ), 'optionalAccess', _41 => _41.status])) {
				const effectHolder = this.effectState.target;
	...
	pickpocket: {
		onAfterMoveSecondary(target, source, move) {
			if (source && source !== target && _optionalChain([move, 'optionalAccess', _42 => _42.flags, 'access', _43 => _43['contact']])) {
	...
	poisontouch: {
		...
		onModifyMove(move) {
			if (!_optionalChain([move, 'optionalAccess', _44 => _44.flags, 'access', _45 => _45['contact']]) || move.target === 'self') return;
			if (!move.secondaries) {
				move.secondaries = [];
			}
	...
	prankster: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _46 => _46.category]) === 'Status') {
	...
	shieldsdown: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _49 => _49.status])) {
	...
	tangledfeet: {
		...
		onModifyAccuracy(accuracy, target) {
			if (typeof accuracy !== 'number') return;
			if (_optionalChain([target, 'optionalAccess', _50 => _50.volatiles, 'access', _51 => _51['confusion']])) {
	...
	triage: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _52 => _52.flags, 'access', _53 => _53['heal']])) return priority + 3;
		},
		name: "Triage",
		rating: 3.5,
		num: 205,
	...
	vitalspirit: {
		...
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'slp') return;
			if (_optionalChain([(effect ), 'optionalAccess', _54 => _54.status])) {
	...
	waterbubble: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _55 => _55.status])) {
	...
	waterveil: {
		...
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'brn') return;
			if (_optionalChain([(effect ), 'optionalAccess', _56 => _56.status])) {
