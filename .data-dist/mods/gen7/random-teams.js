...
var _dex = require('../../../.sim-dist/dex');
 ...
 class RandomGen7Teams extends _randomteams.RandomTeams {
	constructor(format, prng) {
		super(format, prng);RandomGen7Teams.prototype.__init.call(this);RandomGen7Teams.prototype.__init2.call(this);;
	...
	randomSet(
				...
				if (
					...
					if (
						(!counter.get('stab') && !moves.has('nightshade') && !moves.has('seismictoss') && (
							...
							species.requiredMove && movePool.includes(_dex.toID.call(void 0, species.requiredMove))
						) || (
							!counter.get('Normal') &&
		...
		if (!isDoubles) {
			...
			const tier = _dex.toID.call(void 0, species.tier).replace('bl', '');
			...
			level = 70 + Math.floor(((600 - _lib.Utils.clampIntRange(bst, 300, 600)) / 10.34));
		}
	...
	__init() {this.randomFactorySets = require('./factory-sets.json')}

	randomFactorySet(
		species, teamData, tier
	) {
	...
	randomFactoryTeam(side, depth = 0) {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			for (const typeName of this.dex.types.names()) {
				...
				if (_optionalChain([resistanceAbilities, 'access', _ => _[abilityState.id], 'optionalAccess', _2 => _2.includes, 'call', _3 => _3(typeName)]) || !this.dex.getImmunity(typeName, types)) {
	...
	randomBSSFactoryTeam(side, depth = 0) {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			for (const typeName of this.dex.types.names()) {
				// Cover any major weakness (3+) with at least one resistance
				if (teamData.resistances[typeName] >= 1) continue;
				if (_optionalChain([resistanceAbilities, 'access', _4 => _4[abilityState.id], 'optionalAccess', _5 => _5.includes, 'call', _6 => _6(typeName)]) || !this.dex.getImmunity(typeName, types)) {
					// Heuristic: assume that Pokémon with these abilities don't have (too) negative typing.
					teamData.resistances[typeName] = (teamData.resistances[typeName] || 0) + 1;
					if (teamData.resistances[typeName] >= 1) teamData.weaknesses[typeName] = 0;
					continue;
