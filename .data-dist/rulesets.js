"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }// Note: These are the rules that formats use
 ...
 const Rulesets = {
	...
	batonpassclause: {
		...
		onValidateSet(set, format, setHas) {
			...
			for (const moveId of set.moves) {
				...
				if (item.zMove && move.type === item.zMoveType && _optionalChain([move, 'access', _ => _.zMove, 'optionalAccess', _2 => _2.boost])) {
	...
	sketchmonsmovelegality: {
		...
		checkCanLearn(move, species, lsetData, set) {
			...
			if ((set ).sketchMove) {
				return ` already has ${(set ).sketchMove} as a sketched move.\n(${species.name} doesn't learn ${move.name}.)`;
			}
			(set ).sketchMove = move.name;
			return null;
		},
		onValidateTeam(team) {
			const sketches = new _lib.Utils.Multiset();
			for (const set of team) {
				if ((set ).sketchMove) {
	...
	eventmovesclause: {
		...
		onValidateSet(set) {
			const species = this.dex.species.get(set.species);
			const learnsetData = {...(_optionalChain([this, 'access', _3 => _3.dex, 'access', _4 => _4.data, 'access', _5 => _5.Learnsets, 'access', _6 => _6[species.id], 'optionalAccess', _7 => _7.learnset]) || {})};
			let prevo = species.prevo;
			while (prevo) {
				const prevoSpecies = this.dex.species.get(prevo);
				const prevoLsetData = _optionalChain([this, 'access', _8 => _8.dex, 'access', _9 => _9.data, 'access', _10 => _10.Learnsets, 'access', _11 => _11[prevoSpecies.id], 'optionalAccess', _12 => _12.learnset]) || {};
			...
			const problems = [];
			if (_optionalChain([set, 'access', _13 => _13.moves, 'optionalAccess', _14 => _14.length])) {
