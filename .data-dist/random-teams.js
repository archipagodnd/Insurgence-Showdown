"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _optionalChain(ops) { let lastAccessLHS = undefined; let value = ops[0]; let i = 1; while (i < ops.length) { const op = ops[i]; const fn = ops[i + 1]; i += 2; if ((op === 'optionalAccess' || op === 'optionalCall') && value == null) { return undefined; } if (op === 'access' || op === 'optionalAccess') { lastAccessLHS = value; value = fn(value); } else if (op === 'call' || op === 'optionalCall') { value = fn((...args) => value.call(lastAccessLHS, ...args)); lastAccessLHS = undefined; } } return value; }var _dex = require('../.sim-dist/dex');
var _lib = require('../.lib-dist');
var _prng = require('../.sim-dist/prng');
 ...
 class MoveCounter extends _lib.Utils.Multiset {
...
const Hazards = [
 ...
 class RandomTeams {
	...
	constructor(format, prng) {;RandomTeams.prototype.__init.call(this);RandomTeams.prototype.__init2.call(this);
		format = _dex.Dex.formats.get(format);
	...
	randomCCTeam() {
		...
		for (let forme of randomN) {
			...
			if (forme === 'Smeargle') {
				...
				let learnset = _optionalChain([this, 'access', _ => _.dex, 'access', _2 => _2.data, 'access', _3 => _3.Learnsets, 'access', _4 => _4[species.id], 'optionalAccess', _5 => _5.learnset]) && !formes.includes(species.id) ?
			...
			if (species.baseSpecies === 'Wishiwashi') stats = _dex.Dex.species.get('wishiwashischool').baseStats;
	...
	randomHCTeam() {
		...
		for (const forme of randomN) {
			...
			if (this.gen === 6) {
				let evpool = 510;
				do {
					const x = this.sample(_dex.Dex.stats.ids());
					const y = this.random(Math.min(256 - evs[x], evpool + 1));
					evs[x] += y;
					evpool -= y;
				} while (evpool > 0);
			} else {
				for (const x of _dex.Dex.stats.ids()) {
	...
	queryMoves(
		...
		if (!_optionalChain([moves, 'optionalAccess', _6 => _6.size])) return counter;
	...
	shouldCullAbility(
		...
		case 'Contrary': case 'Serene Grace': case 'Skill Link': case 'Strong Jaw':
			return !counter.get(_dex.toID.call(void 0, ability));
		case 'Analytic':
	...
	getLowPriorityItem(
		...
		if (
			...
			(moves.has('boomburst') && Array.from(moves).some(m => _optionalChain([_dex.Dex, 'access', _7 => _7.moves, 'access', _8 => _8.get, 'call', _9 => _9(m), 'access', _10 => _10.boosts, 'optionalAccess', _11 => _11.spe])))
		) return 'Throat Spray';
	...
	randomSet(
		...
		_lib.Utils.sortBy(abilityData, abil => -abil.rating);
	...
	getPokemonPool(
		...
		const exclude = pokemonToExclude.map(p => _dex.toID.call(void 0, p.species));
		const pokemonPool = [];
		for (const id in this.dex.data.FormatsData) {
	...
	randomTeam() {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (_optionalChain([potd, 'optionalAccess', _12 => _12.exists]) && (pokemon.length === 1 || this.maxTeamSize === 1)) species = potd;
	...
	randomBSSFactoryTeam(side, depth = 0) {
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			for (const typeName of this.dex.types.names()) {
				...
				if (_optionalChain([resistanceAbilities, 'access', _13 => _13[abilityState.id], 'optionalAccess', _14 => _14.includes, 'call', _15 => _15(typeName)]) || !this.dex.getImmunity(typeName, types)) {
